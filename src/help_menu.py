...
class HelpMenu:
    def __init__(self, parent):
        ...
        parent.actionUpdate_Frhodo.triggered.connect(
            lambda: self.update_message(show_no_update=True)
        )
        parent.actionAbout.triggered.connect(lambda: self.showAboutBox())

    def newVersionExists(self, current_version):
        response = requests.get(
            "https://api.github.com/repos/Argonne-National-Laboratory/Frhodo/releases/latest"
        )
        github_release_name = response.json()["name"]
        github_release_version = re.findall(
            r"""(\d+(?:\.\d+)*)""", github_release_name
        )[0]
        ...
        current_version = current_version + ["0"] * (
            len(github_version) - len(current_version)
        )
        github_version = github_version + ["0"] * (
            len(current_version) - len(github_version)
        )
