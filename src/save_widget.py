...
class Save_Dialog(QDialog, QApplication):
    def __init__(self, parent):
        ...
        self.var = {'comment': '', 'output_time': np.array([]), 'integrator_time': False, 
                    'parameters': [], 'species': [], 'reactions': [], 'save_plot': True}
        ...
        for widget in [self, self.parameters_list_widget, self.species_list_widget, 
                       self.reactions_list_widget, self.comment_box, self.output_times_box, 
                       self.include_integrator_time_box, self.save_plot_box]:
            widget.installEventFilter(self)
    
    def eventFilter(self, obj, event):  # track last focus excluding pushbuttons
        listWidgets = [self.parameters_list_widget, self.species_list_widget, self.reactions_list_widget]
        ...
        # if output times is empty set integrator_time to True
            ...
            if len(np.fromstring(self.output_times_box.toPlainText(), sep=',', dtype=float)) == 0:
        ...
        # change selection behavior if shift is selectedItems, CURRENTLY NOT FUNCTIONING
            ...
            if event.type() == QtGui.QKeyEvent:
                if event.type() == QtCore.QEvent.KeyPress and event.modifiers() == QtCore.Qt.ShiftModifier:
                    ...
                    self.species_list_widget.setSelectionMode(QListWidget.ExtendedSelection)
                    self.reactions_list_widget.setSelectionMode(QListWidget.ExtendedSelection)
                elif event.type() == QtCore.QEvent.KeyRelease and event.modifiers() == QtCore.Qt.ShiftModifier:
                    print('shift up')
                    self.parameters_list_widget.setSelectionMode(QListWidget.MultiSelection)
                    self.species_list_widget.setSelectionMode(QListWidget.MultiSelection)
                    self.reactions_list_widget.setSelectionMode(QListWidget.MultiSelection)
    ...
    def execute(self, event=None):
        ...
        if self.parent.SIM is None: return                     # if no successful SIM
        if not self.parent.SIM.success: return                 # if prior SIM is not successful, exit
        ...
        if self.focusWidget() is self.comment_box: # select all text if comment box was last edited
    ...
    def _set_variables(self):
        ...
        self.var['parameters'] = [item.text() for item in self.parameters_list_widget.selectedItems()]
        
        # Get species with correct index
        selected_species = [item.text() for item in self.species_list_widget.selectedItems()]
        self.var['species'] = {}
        ...
        selected_rxns = [item.text()[8:] for item in self.reactions_list_widget.selectedItems()]
        self.var['reactions'] = {}
        ...
        t_save = np.fromstring(self.output_times_box.toPlainText(), sep=',', dtype=float)
        ...
        if self.include_time_offset_box.isChecked():
            self.var['output_time_offset'] = self.parent.display_shock['time_offset'] # uses display offset
        else:
