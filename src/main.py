...
class Main(QMainWindow):
    def __init__(self, app, path):
        ...
        self.setWindowIcon(
            QtGui.QIcon(str(self.path["main"] / "UI" / "graphics" / "main_icon.png"))
        )
    ...
    def initialize_settings(self):  # TODO: Solving for loaded shock twice
        ...
        self.load_full_series = (
            self.load_full_series_box.isChecked()
        ...
        self.run_block = (
            False  # Block multiple simulations from running during initialization
        )
    ...
    def load_mech(self, event=None):
        ...
        self.log.clear(
            []
        )  # Clear log when mechanism changes to avoid log errors about prior mech
        mech_load_output = self.mech.load_mechanism(self.path)
        self.log.append(
            mech_load_output["message"], alert=not mech_load_output["success"]
        )
        ...
        if not mech_load_output["success"]:  # if error: update species and return
            ...
            self.log._blink(
                True
            )  # updating_species is causing blink to stop due to successful shock calculation
            return
    ...
    def run_single(self, event=None, t_save=None, rxn_changed=False):
        ...
        SIM_kwargs = {
            ...
            "t_lab_save": t_save,
            "sim_int_f": self.var["reactor"]["sim_interp_factor"],
            "ODE_solver": self.var["reactor"]["ode_solver"],
            "rtol": self.var["reactor"]["ode_rtol"],
            "atol": self.var["reactor"]["ode_atol"],
        }
        ...
        self.SIM, verbose = self.mech.run(
            self.var["reactor"]["name"],
            self.var["reactor"]["t_end"],
            T_reac,
            P_reac,
            mix,
            **SIM_kwargs
