    ...
    def inner(func):
        def wrapper(*args, **kwargs):
            ...
            compile_checkpoints_log(
                file_path=file_path, empty_checkpoints=empty_checkpoints
            )
            return result
    ...
    def inner(func):
        def wrapper(*args, **kwargs):
            ...
            compile_consolidated_log(
                file_path=file_path, compile_checkpoints=compile_checkpoints
            )
            return result
        ...
        if validate_on_fly:
            if not key in valid_optional_fields:
                eprint(
                    "Warning, "
                    + key
                    + " is not an officially supported argument name.  Use of alternative argument names is strongly discouraged."
                )
        if (
            type(kwargs[key]) is not list
        ):  # Make sure single optional args are wrapped in a list
            kwargs[key] = [kwargs[key]]
        new_entry[key] = kwargs[key]
    if (
        unique_id in citations_dict
    ):  # Check for duplicate entries(e.g. from calling the same function twice)
        citations_dict[unique_id] = compare_same_id(
