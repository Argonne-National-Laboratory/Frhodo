...
def combinationGenerator(
    OriginalParameters,
    BaseFactor,
    NumberOfVariationsInEachDirection,
    SpreadType="Addition",
    toFile=False,
    fileName="output",
    headerList="",
    numberOfTimesToWriteToFile=1,
):  # SpreadType can be "Addition" or "Multiplication"
    SpreadType.upper()  # makes userinput uppercase for error protection
    CombinationsSoFar = [OriginalParameters]

    for ParameterP in range(
        len(OriginalParameters)
    ):  # I am Using array indexing, so if the length returned is "21" that means it will iterate across 22 values since 0 and 21 are both values in loop.
        ...
        for CurrentBaseCombinationIndex in range(
            len(CombinationsSoFar)
        ):  # we can't use "for CurrentBaseCombination in CombinationsSoFar" because that would create an infinite loop due to append.
            ...
            if (
                NumberOfVariationsInEachDirection[ParameterP] > 0
            ):  # We're only going to do this for cases where Variations in each direction is non-zero.
                for i in range(
                    1, NumberOfVariationsInEachDirection[ParameterP] + 1
                ):  # If number of vars is 2 for example, then it'll go "1,2" on the i index.
                    ...
                    if SpreadType == "Multiplication":
                        NewCombinationLow[ParameterP] = CurrentBaseCombination[
                            ParameterP
                        ] / (BaseFactor[ParameterP] ** i)
                    ...
                    if SpreadType == "Multiplication":
                        NewCombinationHigh[ParameterP] = CurrentBaseCombination[
                            ParameterP
                        ] * (BaseFactor[ParameterP] ** i)
...
def writeToFile(comboGeneratedList, headerList, filename, numberOfTimesToWriteOutput=1):
    ...
    for line in ListToPrint:
        for b in range(
            1, numberOfTimesToWriteOutput + 1
        ):  # b is an arbitrary counter. we use +1 b/c upper end of range is omitted.
...
if __name__ == "__main__":
    headerList = [
        ...
        "Header3",
        "Header4",
    ]  # array must be seperated by commas to be split into a list
    print(
    ...
    )  # a list using these parameters is created and returned to comboList
