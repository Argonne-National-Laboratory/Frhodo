...
sumNestedAbsValues_2dNested = (
    sumNestedAbsValues  # This really should have been the name originally.
)
        ...
        for elemindex, elem in enumerate(arr1):
            if type(elem) == str:
                if softStringCompare == True:  # if using softStringCompare
                    ...
                    if stringCompare(
                        arr1[elemindex], arr2[elemindex]
                    ):  # Determine if the strings are equal using stringCompare
                ...
                if isinstance(elem, collections.abc.Iterable):
                    subtractNested(
                        arr1[elemindex],
                        arr2[elemindex],
                        subtractionResult[elemindex],
                        relativeTolerance=relativeTolerance,
                        absoluteTolerance=absoluteTolerance,
                        softStringCompare=softStringCompare,
                    )
                    ...
                    if relativeTolerance == None and absoluteTolerance == None:
                        ...
                        if relativeTolerance == None:
                            ...
                            print(
                                "Warning: Can't have absolute tolerance without relative tolerance. Setting relative tolerance to 1.0E-5."
                            )
                        if absoluteTolerance == None:
                            absoluteTolerance = 1.0e-8
                            print(
                                "Warning: Can't have relative tolerance without absolute tolerance. Setting absolute tolerance to 1.0E-8."
                            )
                        # now we do the comparison
                        trueIfTheyAreApproximatelyEqual = np.allclose(
                            arr1[elemindex],
                            arr2[elemindex],
                        ...
                        if trueIfTheyAreApproximatelyEqual == True:
                            subtractionResult[elemindex] = 0
                        if (
                            trueIfTheyAreApproximatelyEqual == False
                        ):  # If they are not equal, we return the actual subtraction result.
                            subtractionResult[elemindex] = (
                                arr1[elemindex] - arr2[elemindex]
                            )
...
def stringCompare(firstString, secondString):
    ...
    standardizedFirstString = re.sub(r"\s+", " ", standardizedFirstString)
    standardizedSecondString = re.sub(r"\s+", " ", standardizedSecondString)
...
def makeAtLeast_2dNested(arr):
    ...
    if (
        (type(arr) == type("str"))
        or ("int" in str(type(arr)))
        or ("float" in str(type(arr)))
    ):  # If it's a string, int, or float, it's not nested.
        nestedArray = [[arr]]
        ...
        if isNestedOrString(arr) == False:
            ...
            if type(arr) == type([]) and type(arr[0]) == type(np.array(0)):
                ...
                nestedArray = copy.deepcopy(
                    arr
                )  # first make a copy, then change what is inside.
...
def convertInternalToNumpyArray_2dNested(
    inputArray,
):  # This is **specifically** for a nested array of the form [ [1],[2,3] ] Such that it is a 1D array of arrays.
    import numpy as np
...
def checkIfStaggered_2dNested(arr):
    try:
        ...
        onesArray = np.matmul(
            arr.transpose(), onesArray
        )  # This is the key line and catches staggered arrays.
        return False
    except:
        return True
