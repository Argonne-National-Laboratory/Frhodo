...
class plotting_functions_class:
    def __init__(
        self, UserInput, samples=False
    ):  # The plots require samples.  Other plot settings are probably plotting-package specific.
    ...
    def mumpce_plots(
        ...
        posterior_mu_vector=0,
        posterior_cov_matrix=0,
        prior_mu_vector=0,
        prior_cov_matrix=0,
    ...
    ):  # Pass empty keyword arguments for important parameters.  That way, warnings may be issued if they are not set.  There is not really a good default for these keyword arguments.  They depend entirely on the nature of the data being plotted.
        mumpceProjectObject = (
            mumpceProject.Project()
        )  # A mumpce project object must be created.
        if len(model_parameter_info) == 0:
            print(
                "Pass the 'model_parameter_info' argument to the mumpce_plots function."
            )
            model_parameter_info = np.array(
                [
                    {"parameter_number": 0, "parameter_name": "Parameter 0"},
        ...
        mumpceSolutionsObject = mumpceSolution.Solution(
            posterior_mu_vector,
            posterior_cov_matrix,
            initial_x=prior_mu_vector,
            initial_covariance=prior_cov_matrix,
        )
        ...
        mumpceProjectObject.plot_pdfs(
            mumpceProjectObject.pairsOfParameterIndices,
...
def sampledParameterHistogramMaker(
    parameterSamples,
    parameterName,
    parameterNamesAndMathTypeExpressionsDict,
    sampledParameterFiguresDictionary,
    sampledParameterAxesDictionary,
):
    parameterIndex = list(parameterNamesAndMathTypeExpressionsDict).index(parameterName)
    (
        sampledParameterFiguresDictionary[parameterName],
        sampledParameterAxesDictionary[parameterName],
    ) = plt.subplots()  # making plt objects
    sampledParameterAxesDictionary[parameterName].hist(
        parameterSamples[:, parameterIndex]
    )  # filling the object with data
    # setting the labels etc. and then exporting.
    axis_font = {
        "size": "16"
    }  # TODO: Make this size a setting that can be changed. #Code was made following answer by "binaryfunt" here https://stackoverflow.com/questions/3899980/how-to-change-the-font-size-on-a-matplotlib-plot
    sampledParameterAxesDictionary[parameterName].set_ylabel("frequency", **axis_font)
    sampledParameterAxesDictionary[parameterName].set_xlabel(
        parameterNamesAndMathTypeExpressionsDict[parameterName], **axis_font
    )

    sampledParameterAxesDictionary[parameterName].tick_params(
        axis="x", labelsize=16
    )  # TODO: make these labels sizes a setting that can be changed.
    ...
    sampledParameterFiguresDictionary[parameterName].savefig(
        "Histogram_sampling_" + parameterName + ".png", dpi=220
    )
...
def makeHistogramsForEachParameter(
    ...
    sampledParameterFiguresDictionary = copy.deepcopy(
        parameterNamesAndMathTypeExpressionsDict
    )  # This must be a deep copy to perserve original.
    sampledParameterAxesDictionary = copy.deepcopy(
        parameterNamesAndMathTypeExpressionsDict
    )  # This must be a deep copy to preserve original.
...
def createSimulatedResponsesPlot(
    x_values,
    listOfYArrays,
    plot_settings={},
    ...
    ):  # This 2d line breaks regular arrays, but works when a 'zero length' array comes in (I don't understand how a zero length array can occur, but it has happened.)
        x_values = np.atleast_2d(x_values)
    if "x_label" not in plot_settings:
        plot_settings["x_label"] = ""
    if "y_label" not in plot_settings:
        plot_settings["y_label"] = ""
    if "legendLabels" not in plot_settings:
        plot_settings["legendLabels"] = ""
    if "figure_name" not in plot_settings:
        plot_settings["figure_name"] = "simulatedResponsesPlot"
    ...
    ax0.set_ylabel(
        plot_settings["y_label"], fontdict=fontdict
    )  # TODO: THis is not yet generalized (will be a function)
    # The error linewidth is going to be set to a thick value if we have a small number of points.
    if "error_linewidth" in plot_settings:
        error_linewidth = plot_settings[
            "error_linewidth"
        ]  # making a convenient local variable.
    if str(error_linewidth).lower() == "auto":
        if len(x_values) == 1:
            ...
            error_linewidth = 10
    if str(error_linewidth).lower() == "none":
        error_linewidth = 0  # This will hide the rror bars if they are not desired.
    if "y_range" in plot_settings:
        ax0.set_ylim(plot_settings["y_range"])
    if (
        len(listOfYArrays) == 3
    ):  # This generally means observed, mu_guess, map, in that order.
        if len(x_values) > 1:  # This means there are enough data to make lines.
            for seriesIndex in range(len(listOfYArrays)):
                ...
                if (
                    len(listOfYUncertaintiesArrays) >= 1
                ):  # If length is >=1, uncertainties for first data set.
                    ax0.errorbar(
                        x_values,
                        listOfYArrays[0],
                        yerr=np.array(listOfYUncertaintiesArrays[0]).flatten(),
                        fmt=".",
                        barsabove=False,
                        markersize=0,
                        ...
                        color="gray",
                        ecolor="lightgray",
                    )  # markersize=0 because we want no marker for experiments data series, just a line..
                ax0.plot(x_values, listOfYArrays[1], "#00A5DF")
                if (
                    len(listOfYUncertaintiesArrays) > 1
                ):  # If length is >1, uncertainties for all data sets
                    ax0.errorbar(
                        x_values,
                        listOfYArrays[1],
                        yerr=np.array(listOfYUncertaintiesArrays[1]).flatten(),
                        fmt=".",
                        barsabove=False,
                        markersize=0,
                        linewidth=error_linewidth,
                        color="gray",
                        ecolor="lightgray",
                    )  # markersize=0 because we want no marker for this.
                ...
                if (
                    ...
                    ax0.errorbar(
                        x_values,
                        listOfYArrays[2],
                        yerr=np.array(listOfYUncertaintiesArrays[2]).flatten(),
                        fmt=".",
                        barsabove=False,
                        markersize=0,
                        linewidth=error_linewidth,
        ...
        if (
            len(x_values) == 1
        ):  # This means there are single points, and we need to make symbols, by adding an "o".
            for seriesIndex in range(len(listOfYArrays)):
        ...
        len(listOfYArrays) == 4
    ):  # This generally means observed, mu_guess, map, mu_app
        if len(x_values) > 1:  # This means there are enough data to make lines.
            for seriesIndex in range(len(listOfYArrays)):
                ax0.plot(x_values, listOfYArrays[0], "g")
                if (
                ...
                if (
                    ...
                    ax0.errorbar(
                        x_values,
                        listOfYArrays[3],
                        yerr=np.array(listOfYUncertaintiesArrays[3]).flatten(),
                        fmt=".",
                        barsabove=False,
                        markersize=0,
                        linewidth=error_linewidth,
        ...
        if (
            ...
            ax0.plot(
                x_values, listOfYArrays[3], "ko"
            )  # k is black. https://matplotlib.org/3.1.0/api/_as_gen/matplotlib.pyplot.plot.html#matplotlib.pyplot.plot
            if (
                len(listOfYUncertaintiesArrays) > 1
            ):  # If length is >1, uncertainties for all data sets
                ax0.errorbar(
        ...
        if (
            len(x_values) == 1
        ):  # This means there are single points, and we need to make symbols.
            for seriesIndex in range(len(listOfYArrays)):
                ax0.plot(x_values[0], listOfYArrays[seriesIndex], "o")
    if plot_settings["legend"] == True:
        ax0.legend(
            plot_settings["legendLabels"]
        )  # legends must be after plots are made.
    fig0.tight_layout()
...
def makeTrisurfacePlot(
    xValues,
    yValues,
    zValues,
    exportFigure=True,
    figure_name="TrisurfacePlot",
    showFigure=True,
):
    from mpl_toolkits.mplot3d import (
        Axes3D,
    )  # Although it does not look like this is called here, the InfoGain plots will fail without this line.
...
def makeMeshGridSurfacePlot(
    XX,
    YY,
    ZZ,
    plot_settings={},
    exportFigure=True,
    figure_name="MeshGridSurfacePlot",
    showFigure=True,
):
    ...
    from mpl_toolkits.mplot3d import (
        Axes3D,
    )  # I am not sure if this line is needed here, it might b.
