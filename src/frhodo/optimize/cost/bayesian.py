...
import frhodo._vendor.CheKiPEUQ as CKPQ
import frhodo._vendor.CheKiPEUQ.UserInput as UserInput
...
def load_into_CheKiPUEQ(
    ...
    pars_upper_bnds=[],
    pars_bnds_exist=[],
    observed_data_lower_bounds=[],
    observed_data_upper_bounds=[],
    weights_data=[],
    pars_uncertainty_distribution="automatic",
    sigma_multiple=3.0,
    num_rate_constants_and_rate_constant_parameters=[],
):
    ...
    if len(num_rate_constants_and_rate_constant_parameters) == 0:
        num_rate_constants_and_rate_constant_parameters = [len(pars_initial_guess), 0]
    ...
    if (
        len(observed_data_lower_bounds) > 0
    ):  # assume that both lower and upper bounds exist on data if there is a lower bounds array provided.
        UserInput.responses[
            "responses_observed_uncertainties"
        ] = extract_larger_delta_and_make_sigma_values(
    ...
    try:
        ...
        ] = weights_data  # (weights_data*np.ones(num_responses)).T
    except:
        print(
            "There was an error in the weightings in CheKiPEUQ_from_Frhodo processing."
        )
    UserInput.model["InputParameterPriorValues"] = pars_initial_guess
    if (
        pars_uncertainty_distribution.lower() == "uniform"
    ):  # make an array of -1 for uncertainties to signify a uniform distribution.
        UserInput.model["InputParametersPriorValuesUncertainties"] = -1 * np.ones(
            len(pars_initial_guess)
        )
    if pars_uncertainty_distribution.lower() == "gaussian":
        UserInput.model[
            "InputParametersPriorValuesUncertainties"
        ] = extract_larger_delta_and_make_sigma_values(
            pars_initial_guess, pars_lower_bnds, pars_upper_bnds, sigma_multiple
        )
    if (
        pars_uncertainty_distribution.lower() == "automatic"
        or pars_uncertainty_distribution.lower() == "auto"
    ):
        num_rate_constants = num_rate_constants_and_rate_constant_parameters[0]
        num_rate_constants_parameters = num_rate_constants_and_rate_constant_parameters[
            1
        ]
        rate_constant_uncertainties = -1 * np.ones(
            num_rate_constants
        )  # by default, use uniform for the rate_constant_uncertainties (signified by -1).
        rate_constant_parameters_uncertainties = extract_larger_delta_and_make_sigma_values(
            pars_initial_guess[num_rate_constants + 0 :],
            pars_lower_bnds[num_rate_constants + 0 :],
            pars_upper_bnds[num_rate_constants + 0 :],
            sigma_multiple,
        )  # this returns a 1 sigma value for a gaussian, assuming that the range indicates a certain sigma_multiple in each direction. The "+0" is to start at next value with array indexing. Kind of like "-1 +1".
        UserInput.model["InputParametersPriorValuesUncertainties"] = np.concatenate(
            (rate_constant_uncertainties, rate_constant_parameters_uncertainties)
        )
    if (
        len(pars_bnds_exist) > 1
    ):  # If this is not a blank list, we're going to check each entry. For anything which has a "False", we are going to set the InputParametersPriorValuesUncertainties value to "-1" to indicate uniform since that means it can't be a Gaussian.
        for exist_index, lower_upper_booleans in enumerate(pars_bnds_exist):
            ...
            if (
                np.sum(lower_upper_booleans) < 2
            ):  # True True will add to 2, anything else does not pass.
...
def get_log_posterior_density(PE_object, parameters):
    # Bounds are enforced upstream by the optimizer; ``runBoundsCheck=False``
    # avoids a second method-mismatched check inside CheKiPEUQ.
    return PE_object.getLogP(parameters, runBoundsCheck=False)


# calculates delta between initial guess and bounds, takes the larger delta, and divides by sigma_multiple to return sigma.
def extract_larger_delta_and_make_sigma_values(
    initial_guess, lower_bound, upper_bound, sigma_multiple
):
...
def get_varying_rate_vals_and_bnds(rate_vals, rate_bnds):
    ...
    for bounds_index, bounds_pair in enumerate(rate_bnds):
        if (
            np.isnan(bounds_pair[0]) == False or np.isnan(bounds_pair[1]) == False
        ):  # this means there is a pair of bounds.
            varying_rate_vals_indices.append(bounds_index)  # store this index.
            varying_rate_vals_initial_guess.append(
                rate_vals[bounds_index]
            )  # append current rate_val
            varying_rate_vals_lower_bnds.append(
                rate_bnds[bounds_index][0]
            )  # append current lower bound
            varying_rate_vals_upper_bnds.append(
                rate_bnds[bounds_index][1]
            )  # append current upper bound
    return (
        varying_rate_vals_indices,
        varying_rate_vals_initial_guess,
        varying_rate_vals_lower_bnds,
...
def get_consolidated_parameters_arrays(
    ...
    rate_constants_bnds_exist = np.array(
        rate_constants_bnds_exist, dtype=bool
    )  # Can't flatten() because these have to be retained as pairs;

    # Now we concatenate those with the rate_constant_parameters arrays.
    pars_values = np.concatenate(
        (pars_values, np.array(rate_constants_parameters_values).flatten())
    )
    pars_lower_bnds = np.concatenate(
        (pars_lower_bnds, np.array(rate_constants_parameters_lower_bnds).flatten())
    )
    pars_upper_bnds = np.concatenate(
        (pars_upper_bnds, np.array(rate_constants_parameters_upper_bnds).flatten())
    )
    pars_bnds_exist = np.concatenate(
        (
            rate_constants_bnds_exist,
            np.array(rate_constants_parameters_bnds_exist, dtype=bool),
        )
    )  # Can't flatten() because these have to be retained as pairs;
...
class CheKiPEUQ_Frhodo_interface:
    def __init__(
        ...
        self.bayes_unc_sigma = float(bayes_unc_sigma)
        self.Bayesian_dict = {}
        self.Bayesian_dict["pars_uncertainty_distribution"] = bayes_dist_type
        self.Bayesian_dict["rate_constants_initial_guess"] = deepcopy(
            rxn_rate_opt["x0"]
        )
        self.Bayesian_dict["rate_constants_lower_bnds"] = deepcopy(
            rxn_rate_opt["bnds"]["lower"]
        )
        self.Bayesian_dict["rate_constants_upper_bnds"] = deepcopy(
            rxn_rate_opt["bnds"]["upper"]
        )
        num_rate_consants = len(self.Bayesian_dict["rate_constants_initial_guess"])
        self.Bayesian_dict["rate_constants_bnds_exist"] = np.array(
            np.ones((num_rate_consants, 2)), dtype=bool
        )

        self.Bayesian_dict["rate_constants_parameters_changing"] = deepcopy(coef_opt)
        ...
        for rxn_coef in rxn_coef_opt:
            self.Bayesian_dict[
                ...
                (
                    ...
                    deepcopy(rxn_coef["coef_x0"]),
                ),
                axis=None,
            )
            self.Bayesian_dict["rate_constants_parameters_lower_bnds"] = np.concatenate(
                (
                    self.Bayesian_dict["rate_constants_parameters_lower_bnds"],
                    deepcopy(rxn_coef["coef_bnds"]["lower"]),
                ),
                axis=None,
            )
            self.Bayesian_dict["rate_constants_parameters_upper_bnds"] = np.concatenate(
                (
                    self.Bayesian_dict["rate_constants_parameters_upper_bnds"],
                    deepcopy(rxn_coef["coef_bnds"]["upper"]),
                ),
                axis=None,
            )
            self.Bayesian_dict["rate_constants_parameters_bnds_exist"].extend(
                list(deepcopy(rxn_coef["coef_bnds"]["exist"]))
            )  # we can't use append because this is a list of lists/array, and we want parallel construction.
        ...
        (
            Bayesian_dict["pars_initial_guess"],
            Bayesian_dict["pars_lower_bnds"],
            Bayesian_dict["pars_upper_bnds"],
            Bayesian_dict["pars_bnds_exist"],
            Bayesian_dict["unbounded_indices"],
        ) = get_consolidated_parameters_arrays(
        ...
        if len(Bayesian_dict["unbounded_indices"]) > 0:
            Bayesian_dict["pars_initial_guess_truncated"] = remove_unbounded_values(
                Bayesian_dict["pars_initial_guess"], Bayesian_dict["unbounded_indices"]
            )
            Bayesian_dict["pars_lower_bnds_truncated"] = remove_unbounded_values(
                Bayesian_dict["pars_lower_bnds"], Bayesian_dict["unbounded_indices"]
            )
            Bayesian_dict["pars_upper_bnds_truncated"] = remove_unbounded_values(
                Bayesian_dict["pars_upper_bnds"], Bayesian_dict["unbounded_indices"]
            )
            Bayesian_dict["pars_bnds_exist_truncated"] = remove_unbounded_values(
                Bayesian_dict["pars_bnds_exist"], Bayesian_dict["unbounded_indices"]
            )
        else:
            Bayesian_dict["pars_initial_guess_truncated"] = Bayesian_dict[
                "pars_initial_guess"
            ]
            Bayesian_dict["pars_lower_bnds_truncated"] = Bayesian_dict[
                "pars_lower_bnds"
            ]
            Bayesian_dict["pars_upper_bnds_truncated"] = Bayesian_dict[
                "pars_upper_bnds"
            ]
            Bayesian_dict["pars_bnds_exist_truncated"] = Bayesian_dict[
                "pars_bnds_exist"
            ]
    ...
    def evaluate(
        ...
        Bayesian_dict["pars_current_guess"] = np.concatenate(
            (
                Bayesian_dict["rate_constants_current_guess"],
                Bayesian_dict["rate_constants_parameters_current_guess"],
            )
        )
        Bayesian_dict["last_obs_sim_interp"] = np.concatenate(
            output_dict["obs_sim_interp"], axis=0,
        )
        Bayesian_dict["observed_data"] = np.concatenate(
            output_dict["obs_exp"], axis=0,
        )
        obs_bounds_concat = np.concatenate(output_dict["obs_bounds"], axis=0)
        if obs_bounds_concat.ndim != 2 or obs_bounds_concat.shape[0] == 0:
            # Every shock's exp window collapsed (typically because the
            # optimizer's current parameters made the sim degenerate, so
            # the residual func filtered all points out). No data → no
            # likelihood is defined; signal "undefined" with inf so the
            # optimizer rejects this point rather than chasing a
            # fabricated finite penalty.
            return np.inf
        obs_data_bnds = obs_bounds_concat.T
        Bayesian_dict["observed_data_lower_bounds"] = obs_data_bnds[0]
        Bayesian_dict["observed_data_upper_bounds"] = obs_data_bnds[1]
        Bayesian_dict["weights_data"] = bayesian_weights

        def get_last_obs_sim_interp(
            varying_rate_vals,
        ):  # A. Savara added this. It needs to be here.
            try:
        ...
        Bayesian_dict[
            "simulation_function"
        ] = get_last_obs_sim_interp  # using wrapper that just returns the last_obs_sim_interp
        ...
        if np.shape(Bayesian_dict["weights_data"]) != np.shape(
            Bayesian_dict["observed_data"]
        ):
            raise Exception(
                "CheKiPEUQ Error: Shape of weights does not match observed data"
            )

        num_rate_constants = len(Bayesian_dict["rate_constants_initial_guess"])
        num_rate_constant_parameters = len(
            Bayesian_dict["rate_constants_parameters_initial_guess"]
        )
        ...
        CheKiPEUQ_PE_object = load_into_CheKiPUEQ(
            ...
            pars_uncertainty_distribution=Bayesian_dict[
                "pars_uncertainty_distribution"
            ],
            num_rate_constants_and_rate_constant_parameters=[
                num_rate_constants,
                num_rate_constant_parameters,
            ],
            sigma_multiple=self.bayes_unc_sigma,
        ...
        # TODO: This might be improper way to handle all paramters having bounds
        if len(Bayesian_dict["unbounded_indices"]) > 0:
            Bayesian_dict["pars_current_guess_truncated"] = remove_unbounded_values(
                Bayesian_dict["pars_current_guess"], Bayesian_dict["unbounded_indices"]
            )
        else:
            Bayesian_dict["pars_current_guess_truncated"] = Bayesian_dict[
                "pars_current_guess"
            ]

        log_posterior_density = get_log_posterior_density(
            CheKiPEUQ_PE_object, Bayesian_dict["pars_current_guess_truncated"]
        )
        neg_log_posterior_density = (
            -1 * log_posterior_density
        ...
        if iteration_num == 0 and "obj_fcn_initial" not in Bayesian_dict:
            ...
            obj_fcn = (
                (neg_log_posterior_density - Bayesian_dict["obj_fcn_initial"])
                / np.abs(Bayesian_dict["obj_fcn_initial"])
                * 100
            )
