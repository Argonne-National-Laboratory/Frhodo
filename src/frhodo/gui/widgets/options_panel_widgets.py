...
"""Options-panel widgets: directory, shock, reactor, mix, weight, uncertainty tables.
...
from qtpy.QtWidgets import (
    QCheckBox, QFileDialog, QHeaderView, QTabBar, QTabWidget,
...
class Initialize(QtCore.QObject):
    """Bootstraps every options-panel widget on the main window.

    Called once at app start; instantiates the per-section controllers
    (:class:`Directories`, :class:`Shock_Settings`, ...) and wires
    them onto the existing Qt widgets defined in the ``.ui`` file.
    """
    ...
    def __init__(self, parent):
        ...
        handler = GuiLogHandler(parent.log)
        handler.setFormatter(logging.Formatter("%(message)s"))
        root = logging.getLogger("frhodo")
        ...
        parent.random_t_unc_box.stateChanged.connect(parent.update_user_settings)
        ...
        # Right-click "Auto-fit" on the time-offset boxes
        for box in (parent.time_offset_box, parent.time_offset_twin_box):
            box.setContextMenuPolicy(QtCore.Qt.CustomContextMenu)
            box.customContextMenuRequested.connect(
                self._show_time_offset_menu,
            )
    ...
    def _show_time_offset_menu(self, pos):
        ...
        auto_action.setChecked(bool(parent.time_uncertainty.auto_fit))
        auto_action.toggled.connect(self._set_auto_fit)
        menu.exec_(box.mapToGlobal(pos))

    def _set_auto_fit(self, enabled: bool):
        """Toggle the (series-level) auto-fit flag; trigger an immediate fit when turned on.

        Triggers a fresh ``run_single`` so the fit operates on a current
        SIM (the post-sim hook calls ``apply_auto_fit_time_offset``);
        falls back to a direct ``apply_auto_fit_time_offset`` call when
        a run is blocked (e.g., no mechanism loaded yet) so the
        toggle still does something when SIM is already current.
        """
        parent = self.parent()
        parent.time_uncertainty.auto_fit = bool(enabled)
        if not enabled:
            return
...
_AUTO_FIT_FALLBACK_BOUND_FRACTION = 0.5


def apply_auto_fit_time_offset(parent):
    """Recompute the time offset for the displayed shock.

    Called both interactively (right-click Auto-fit toggle) and from
    ``run_single`` after the simulation updates. No-op when
    ``time_uncertainty.auto_fit`` is off or the SIM is empty.
    ...
    if shock is None:
        return
    SIM = getattr(parent, "SIM", None)
    if SIM is None:
        return
    try:
        ind_var = np.asarray(SIM.independent_var).flatten()
        obs_sim = np.asarray(SIM.observable).flatten()
    except (AttributeError, ValueError):
        return
    if ind_var.size < 2 or obs_sim.size != ind_var.size:
        return

    exp_data = getattr(shock, "exp_data", None)
    if exp_data is None or len(exp_data) == 0:
        return

    t_exp = np.asarray(exp_data[:, 0], dtype=float)
    obs_exp = np.asarray(exp_data[:, 1], dtype=float)
    # Always recompute from the shock's current weight parameters rather
    # than reading ``shock.weights`` — that array can be empty (cleared
    # by ``clear_shocks`` on shock switch when load_full_series is off)
    # or stale (user edited weight_shift / weight_k since the last plot
    # update). ``calcIntegral=False`` skips the cumulative-trapz pass and
    # leaves ``shock.normalized_weights`` untouched.
    try:
        weights = np.asarray(
            parent.series.weights(t_exp, shock=shock, calcIntegral=False),
            dtype=float,
        )
    except Exception:
        weights = np.ones_like(t_exp)
    if weights.size != t_exp.size:
        weights = np.ones_like(t_exp)

    span = float(t_exp[-1] - t_exp[0])
    if span <= 0:
        return

    if parent.run_control.optimize_running:
        # The optimizer's CostFunction is already solving the per-shock
        # offset on every iteration. Display its baseline so the box
        # tracks the optimizer's anchor; running our own solve here would
        # both fight the optimizer and flicker per iteration.
        t_offset = float(shock.opt_time_offset)
    else:
        bound = _AUTO_FIT_FALLBACK_BOUND_FRACTION * span
        shift_arr = getattr(shock, "weight_shift", None)
        if shift_arr is not None and len(shift_arr) >= 2:
            fall_pct = float(shift_arr[1])
            if np.isfinite(fall_pct):
                bound = max(0.0, fall_pct / 100.0 * span)
        f_interp = CubicSpline(ind_var, obs_sim)

        t_offset = _solve_t_unc(
            f_interp=f_interp,
            t_exp=t_exp,
            obs_exp=obs_exp,
            weights=weights,
    ...
    # Use the spinbox's rounded value as the canonical time offset so the
    # box-value-vs-shock.time_offset equality check in update_user_settings
    # is stable under spinbox display precision.
    synced_t_offset = parent.time_offset_box.value() * t_unit_conv
    shock.time_offset = synced_t_offset
    parent.time_uncertainty.offset = synced_t_offset

    sim_item = getattr(parent.plot.signal.ax[1], "item", {}).get("sim_data")
    if sim_item is not None:
        sim_item.set_xdata(np.asarray(SIM.independent_var) + synced_t_offset)
        parent.plot.signal.fig.canvas.draw_idle()


class Directories(QtCore.QObject):
    """Top of the options panel — experimental data / mechanism / save directories."""
    ...
    def set_load_full_set(self, event=None):
        parent = self.parent()
        parent.load_state.load_full_series = parent.load_full_series_box.isChecked()
        if event:
            parent.series.load_full_series()
            # parent.series_viewer._update(load_full_series = parent.load_state.load_full_series)


class Shock_Settings(QtCore.QObject):
    """Shock-conditions block — T1/P1/u1 inputs and zone selection."""
    ...
    def set_shock_value_box(self, var_type):
        ...
        value = getattr(parent.display_shock, var_type)
    ...
    def _shock_value_changed(self, event):
        ...
        setattr(
            parent.display_shock,
            var_type,
            self.convert_units(event, units, unit_dir="in"),
        )
    ...
    def solve_postshock(self, var_type):
        ...
        IC = [getattr(parent.display_shock, key) for key in ("T1", "P1")]
        if (
            not np.isnan(IC).any() and len(parent.display_shock.thermo_mix) > 0
        ):  # if T1, P1, thermo_mix all valid
            IC = [
                getattr(parent.display_shock, key)
                for key in ("u1", "T2", "P2", "T5", "P5")
            ]
        ...
        shock = ShockJumpSolver(parent.mech.gas, shock_vars)
        ...
        # Update new values and run sim — copy back the zone variables we
        # did NOT supply as inputs.
        zone_vars = {"u1", "T1", "P1", "T2", "P2", "T5", "P5"}
        for var in zone_vars - set(shock_vars.keys()):
            setattr(parent.display_shock, var, getattr(shock.res, var))
...
class Reactor_Settings(QtCore.QObject):
    """Reactor-settings block — reactor type, ODE solver, tolerances, end time."""
    ...
    def update_reactor_variables(self, event=None):
        ...
        parent.reactor_state.solve_energy = parent.solve_energy_box.isChecked()
        parent.reactor_state.frozen_comp = parent.frozen_comp_box.isChecked()

        # Set Simulation time
        if "μs" in parent.end_time_units_box.currentText():
            t_unit_conv = parent.reactor_state.t_unit_conv = 1e-6
        elif "ms" in parent.end_time_units_box.currentText():
            t_unit_conv = parent.reactor_state.t_unit_conv = 1e-3
        elif "s" in parent.end_time_units_box.currentText():
            t_unit_conv = parent.reactor_state.t_unit_conv = 1

        t_unit = parent.end_time_units_box.currentText()
        parent.time_offset_box.setSuffix(" " + t_unit)

        parent.reactor_state.ode_solver = parent.ODE_solver_box.currentText()
        parent.reactor_state.ode_rtol = 10 ** parent.ODE_rtol_box.value()
        parent.reactor_state.ode_atol = 10 ** parent.ODE_atol_box.value()
        parent.reactor_state.t_end = parent.end_time_value_box.value() * t_unit_conv
        parent.reactor_state.sim_interp_factor = parent.sim_interp_factor_box.value()
    ...
    def update_reactor_choice(self, event=None):
        ...
        parent.reactor_state.name = parent.reactor_select_box.currentText()
        ...
        if parent.reactor_state.name == "Incident Shock Reactor":
            parent.zero_d_choice_frame.hide()
            parent.solver_frame.show()
            parent.series.set("zone", 2)
        elif "0d Reactor" in parent.reactor_state.name:
...
class Mix_Table(QtCore.QObject):
    """Mixture-composition table editor."""
    ...
    def __init__(self, parent):
        ...
        stylesheet = [
            ...
            "background-color: qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1,"  # windows 7 gradient
    ...
    def update_species(self):
        ...
        gas = getattr(parent.mech, "gas", None)
        if gas is not None and hasattr(gas, "species_names"):
            self.setItems(gas.species_names, exp_mix=exp_mix, alias=species_alias)
        else:
            self.setItems([], exp_mix=exp_mix, alias=species_alias)


class Weight_Parameters_Table(QtCore.QObject):
    """Weight-profile parameters (peak, floors, time of rise/fall, growth rates)."""
...
class Tables_Tab(QtCore.QObject):
    def __init__(self, parent):
        ...
        parent.tree = mech_widget.Tree(parent)
        parent.tree_thermo = thermo_widget.Tree(parent)
...
class Log:
    """Log-tab controller; appends messages and blinks the tab on alerts."""
...
class Optimization(QtCore.QObject):
    """Optimization-tab controller — algorithm/stage selectors + start button."""
