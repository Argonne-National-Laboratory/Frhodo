...
"""File-writing back-end for the Save dialog: trace tables + mechanism dumps."""
import numpy as np
...
class Save:
    ...
    Reads the state from the parent's :class:`SaveDialogState`; emits
    one ``.txt`` table per selected parameter group plus optional
    mechanism and plot dumps.
    """
    ...
    def make_table(self, labels, data, name="", sig_fig=3):
        ...
        ]  # replace regular spaces with no-break space for excel
        num_col = np.shape(data)[0]
        if isinstance(sig_fig, list):  # Checks if sig_fig is a list
            if (
                np.shape(sig_fig)[0] < num_col
            ):  # if number of values is less than number of inputs, duplicate last entry
                sig_fig.append(np.ones(num_col - np.shape(sig_fig)[0]) * sig_fig[-1])
            sig_fig = np.array(sig_fig).astype(int)
        else:
            sig_fig = (np.ones(num_col) * sig_fig).astype(
                int
            )  # if not a list, set all sig_fig as input sig_fig
        ...
        table = tabulate(
            table_fill, stralign="center", numalign="center", tablefmt="plain"
        )
        table = table.splitlines()

        for n, row in enumerate(
            table
        ):  # Replace leading spaces with no-break space for excel
    ...
    def write_table(
        self, path, table, line_start=0
    ):  # Notepad has a maximum line length of 1024 characters
    ...
    def all(self, SIM, save_var, units="CGS"):
        ...
        for parameter in save_var["parameters"]:
            ...
            if (
                parameter == "Density Gradient"
            ):  # if Density Gradient is being saved, save Total Density Gradient
        ...
        if save_var["save_plot"]:
            ...
            parent.plot.signal.fig.savefig(
                parent.path["Plot.png"], dpi="figure", format="png"
            )

    def sim_parameter(self, SIM, save_var, parameter, path, units="CGS"):
        idx = self.save_indices
        data = (
            np.array(SIM.t_lab(units=units)[idx]).T + save_var["output_time_offset"]
        )  # add time offset to all

        header = ["time [s]"]
        sub_type = sub_types_for_display(parameter)
        SIM_name = base_sim_name_for_display(parameter)
        if sub_type is None:  # TODO: save in one file?
            ...
            if "total" in sub_type:
                header.append("Total")
                data = np.vstack(
                    (data, getattr(SIM, SIM_name + "_tot")(units=units)[idx])
                )
            ...
            if len(save_var["species"]) > 0:
                for species_idx, species_name in save_var["species"].items():
                    ...
                    data = np.vstack(
                        (data, getattr(SIM, SIM_name)(units=units)[:, idx][species_idx])
                    )
            ...
            if len(save_var["reactions"]) > 0:
                for rxn_idx, rxn_eqn in save_var["reactions"].items():
                    ...
                    data = np.vstack(
                        (data, getattr(SIM, SIM_name)(units=units)[:, idx][rxn_idx])
                    )
    ...
    def sim_density_gradient(self, SIM, save_var, path, units="CGS"):
        ...
        data = (
            np.array(SIM.t_lab(units=units)).T + save_var["output_time_offset"]
        )  # add time offset to all
    ...
    def sim_log_txt(self, save_var):
        ...
        if save_var["output_time_offset"] != 0.0:
            ...
            if (
                len(comment) == 0 or comment.isspace()
            ):  # if no comment exists, make this comment
                comment = addComment
            else:
                comment = comment + "\n" + addComment
    ...
    def chemkin_format(self, gas=None, path=None):
        if path is None:
            path = self.path["Mech.ck"]
        self.parent.mech.to_chemkin(path)
