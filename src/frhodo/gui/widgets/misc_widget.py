...
from qtpy.QtWidgets import (
    QApplication, QComboBox, QCompleter, QLabel, QMenu, QShortcut,
...
class ScientificDoubleSpinBox(QtWidgets.QDoubleSpinBox):
    ...
    def __init__(self, reset_popup=True, *args, **kwargs):
        ...
        self.setDecimals(
            int(np.floor(np.log10(sys.float_info.max)))
        )  # big for setting value
        ...
        if reset_popup:
            ...
            # Set Shortcuts
            shortcut_fcn_pair = [
                ["Ctrl+R", lambda: self._reset()],
                ["Ctrl+C", lambda: self._copy()],
                ["Ctrl+V", lambda: self._paste()],
            ]
            for (
                shortcut,
                fcn,
            ) in shortcut_fcn_pair:  # TODO: need to fix hover shortcuts not working
                QShortcut(
    ...
    def _popup_menu(self, event):
        ...
        popup_menu.addAction(
            "Set Reset Value", lambda: self._set_reset_value(self.value())
        )
    ...
    def _reset(self, silent=False):
        self.blockSignals(
            True
        )  # needed because shortcut isn't always signalling valueChanged.emit
    ...
    def keyPressEvent(self, event):
        ...
        super(ScientificDoubleSpinBox, self).keyPressEvent(
            event
    ...
    def textFromValue(self, value):
        ...
        if "g" in self.numFormat:
            ...
            if (
                abs(OoM(value)) < self.strDecimals
                and self.decimals() < self.strDecimals
            ):
                string = "{:.{dec}{numFormat}}".format(
                    value,
                    dec=int(abs(OoM(value))) + 1 + self.decimals(),
                    numFormat=self.numFormat,
                )
            else:
                string = "{:.{dec}{numFormat}}".format(
                    value, dec=self.strDecimals, numFormat=self.numFormat
                )
    ...
    def stepBy(self, steps):
        ...
        old_val = float(text)
        if (
            self.numFormat == "g" and abs(OoM(old_val)) < self.strDecimals
        ):  # use integer step when value is within strDecimals range
...
class SearchComboBox(QComboBox):
    ...
    def setNewStyleSheet(self, down_arrow_path):
        ...
        # removes arrow border
        stylesheet = [
            "QComboBox { color: black;  font-size: " + str(font_size) + "px;",
...
class CheckableSearchComboBox(ItemSearchComboBox):
    ...
    def sizeHint(self):
        ...
        if type(self.view()) is QTreeView:  # if the view is a QTreeView
            for i in range(
                self.view().header().count()
            ):  # add size hint for each column
