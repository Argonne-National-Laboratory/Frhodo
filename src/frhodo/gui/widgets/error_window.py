...
class Error_Window(QDialog):
    def __init__(self, app, path, error_text):
        super().__init__()
        uic.loadUi(str(path["package"] / "ui" / "error_window.ui"), self)
        self.setWindowIcon(
            QtGui.QIcon(str(path["package"] / "ui" / "graphics" / "main_icon.png"))
        )
        self.setWindowFlags(
            QtCore.Qt.Window
...
def excepthookDecorator(app, parent_path, on_error=None):
    path = parent_path
    ...
    def excepthook(type, value, tback):
        ...
        log_handler = RotatingFileHandler(
            path["log"],
            mode="a",
            maxBytes=1 / 2 * 1024 * 1024,  # maximum of 512 kB
            backupCount=1,
            encoding=None,
            delay=0,
        )  # maximum of 2 error files
