...
from qtpy.QtWidgets import (
    QAbstractItemView, QAction, QCheckBox, QGridLayout, QHBoxLayout,
    QHeaderView, QMenu, QShortcut, QTableWidget, QTableWidgetItem,
    QTreeWidget, QTreeWidgetItemIterator, QWidget,
)
...
class Series_Viewer:
    ...
    Owns the two-pane series widget — a tree of series on the left and
    a per-series data table on the right. Wires the "Add series"
    button and forwards row selections to the rest of the GUI.
    """
    ...
    def updateSizeHint(self, visible_items=2):
        ...
        treeHeight = (
            self.tree.maximumViewportSize().height() - self.tree.header().height()
        )
        if len(self.data_table) > 0:
            if len(self.data_table) <= (
                visible_items + 1
            ):  # limit size to show other sets
            ...
            for n, L2 in enumerate(self.L2):
                ...
                tableHeight = (
                    self.data_table[n].getSize().height() + self.collapsedItemHeight
                )
    ...
    def update(self, shock_idx=None):
        if not self.data_table:
            return  # in case no series have been added
...
class DataSetsTable(QTableWidget):
    ...
    def _update(self, shock_idx=None):
        ...
        for idx in shock_idxs:
            ...
            if "exp_data" in shock_data.err:
                self._toggle_checkbox(idx=idx, bool=False)
                continue

            # Check that proper variables exist
            IC = [getattr(shock_data, key) for key in ["T1", "P1", "u1"]]
            if np.isnan(IC).any() or len(shock_data.thermo_mix) == 0:
                continue

            parent.series.thermo_mix(shock=shock_data)

            shock_vars = {
                "T1": shock_data.T1,
                "P1": shock_data.P1,
                "u1": shock_data.u1,
                "mix": shock_data.thermo_mix,
            }
            ...
            if not shock.success:
                self._toggle_checkbox(idx=idx, bool=False)
                shock_data.err.append("SIM Failure")
                continue

            for var in ["rho1", "T2", "P2", "u2", "T5", "P5"]:
                setattr(shock_data, var, getattr(shock.res, var))
            ...
            for n, var in enumerate(self.column_names):
                if var in ["T1", "P1", "P4", "T2", "P2", "T5", "P5"]:
                    ...
                    display_value = self.convert_units(
                        getattr(shock_data, var), unit, unit_dir="out"
                    )
    ...
    def _selection_change(self):
        ...
        if (
            self.series_holder.last_selected_table is None
        ):  # for clearing other tables if selected
        ...
        if all(x == selected_rows[0] for x in selected_rows):  # only 1 row is selected
            if selected_columns == list(range(num_columns)):  # full row is selected
                ...
                if parent.path["exp_main"] != self.exp_path:
                    parent.exp_main_box.setText(
                        str(self.exp_path)
                    )  # set to selected table's exp path
    ...
    def _toggle_checkbox(self, event=None, **kwargs):
        ...
        def shockError(idx):
            shock_err = list(
                dict.fromkeys(self.shock[idx].err)
            )  # remove duplicate values
            if len(shock_err) > 0 and shock_err != [
                "raw_data"
            ]:  # ignore error if raw data doesn't exist
    ...
    def keyPressEvent(self, event):
        ...
        super(DataSetsTable, self).keyPressEvent(
            event
        )  # don't want to overwrite all shortcuts
    ...
    def _popup_menu(self, event):
        ...
        copyShockNumAction.triggered.connect(
            lambda event: setCopyShockNumAction(self, event)
        )
