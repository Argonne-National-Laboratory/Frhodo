...
class Draggable:
    ...
    def _on_press(self, event):
        if event.inaxes != self.dr_obj.axes:
            return
        if Draggable.lock is not None:
            return
        if self.dr_obj.axes is None or self.parent.toolbar.mode:
            return  # don't drag if something else is selected
        ...
        if self.press_fcn is not None:
            x = {
                ...
                "press": event.xdata,
                "new": event.xdata,
                "press_new": event.xdata,
            }
            y = {
                "0": y0,
                "press": event.ydata,
                "new": event.ydata,
                "press_new": event.ydata,
            }
            self.press_fcn(x, y)
    ...
    def _on_motion(self, event):
        ...
        QtCore.QTimer.singleShot(_MOTION_TICK_MS, self._motion_tick)
    ...
    def _motion_tick(self):
        ...
        if event is None or Draggable.lock is not self:
            return
    ...
    def _on_release(self, event):
        ...
        parent._draw_event_signal = parent.canvas.mpl_connect(
