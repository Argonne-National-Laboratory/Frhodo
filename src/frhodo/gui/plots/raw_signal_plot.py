...
class Plot(Base_Plot):
    ...
    def info_table_text(self, preshock, postshock, prec=2):
        ...
        shock_zone = 2
        table = [
            ["pre-shock", "{:.{dec}f}".format(preshock, dec=prec)],
    ...
    def create_canvas(self):
        ...
        self.ax[0].item["exp_data"] = self.ax[0].scatter(
            [],
            [],
            color="#0C94FC",
            facecolors="#0C94FC",
        ...
        self.ax[0].item["start_divider"] = self.ax[0].add_line(
            mpl.lines.Line2D(
                [],
                [],
                marker="$" + "\u2336" + "$",
        ...
        self.ax[0].item["start_avg_l"] = self.ax[0].add_line(
            mpl.lines.Line2D([], [], ls="--", c="0")
        )
        self.ax[0].item["end_divider"] = self.ax[0].add_line(
            mpl.lines.Line2D(
                [],
                [],
                marker="$" + "\u2336" + "$",
        ...
        self.ax[0].item["end_avg_l"] = self.ax[0].add_line(
            mpl.lines.Line2D([], [], ls="--", c="0")
        )

        self.ax[0].item["textbox"] = self.ax[0].text(
            0.98,
            0.98,
            self.info_table_text(0, 0),
            fontsize=10,
        ...
        self.fig.subplots_adjust(
            left=0.06, bottom=0.065, right=0.98, top=0.97, hspace=0, wspace=0.12
        )
        ...
        for pair in draggable_items:
            ...
            self.ax[n].item[name].draggable = Draggable(
                self, self.ax[n].item[name], update_fcn
            )

    def draggable_update_fcn(self, name, x, y):
        if self.parent.display_shock.raw_data.size == 0:
            return
    ...
    def update(self, estimate_ind=True, update_lim=False):
        ...
        def set_xy(plot, x, y):
            # matplotlib >=3.7 requires sequences; wrap scalar/array uniformly
            plot.set_xdata(np.atleast_1d(x))
            plot.set_ydata(np.atleast_1d(y))
        ...
        if estimate_ind:
            ...
            self.end_ind = -estimateInd(
                data[::-1], frac=0.1, alpha=0.001
            )  # 2-tail 99.95%
        ...
        set_xy(
            self.ax[0].item["start_avg_l"],
            (t[0], t[start_ind]),
            start_avg * np.ones((2, 1)),
        )

        end_avg = np.mean(data[end_ind:])
        set_xy(self.ax[0].item["end_divider"], (t[end_ind]), (end_avg))
        set_xy(
            self.ax[0].item["end_avg_l"], (t[end_ind], t[-1]), end_avg * np.ones((2, 1))
        )
