...
"""Shared base for every matplotlib plot widget — toolbar, scales, cursor."""
...
from frhodo.gui.plots.custom_mplscale import *
from frhodo.gui.plots.custom_mpl_ticker_formatter import *
...
class Base_Plot(QtCore.QObject):
    """Common matplotlib host for every Frhodo plot tab.

    Subclasses (:class:`signal_plot.Plot`, :class:`raw_signal_plot.Plot`,
    :class:`sim_explorer_plot.Plot`, :class:`optimization_plot.Plot`)
    add their per-tab axes, traces, and event handlers; this base
    owns the figure, toolbar, scale registration, and shared cursor.
    """
    ...
    def set_xlim(self, axes, x):
        if not self.autoScale[0]:
            return  # obey autoscale right click option
        ...
        if "exp_data" in axes.item:
            ...
            if "sim_data" in axes.item and hasattr(axes.item["sim_data"], "raw_data"):
                if axes.item["sim_data"].raw_data.size > 0:
                    data["x"] = np.append(
                        data["x"], axes.item["sim_data"].raw_data[:, 0]
                    )
    ...
    def _set_scale(self, coord, type, event, update_xylim=False):
        ...
        for shared in shared_axes:
            shared.scale[coord] = dict.fromkeys(
                shared.scale[coord], False
            )  # sets all types: False
        ...
        if type == "linear":
            ...
            str = 'axes.set_{0:s}scale("{1:s}", nonpositive="mask")'.format(
                coord, "log"
            )
        elif type == "abslog":
    ...
    def key_press(self, event):
        ...
        # elif event.key == 'shift':
            ...
            event.key == "x"
        ):  # Does nothing, would like to make sticky constraint zoom/pan
    ...
    def NavigationToolbar(self, *args, **kwargs):
        ...
        self.toolbar = CustomNavigationToolbar(
            self.canvas, self.widget, coordinates=True
        )
