...
from frhodo.simulation.shock.reactor_output import sub_types_for_display
...
class Observable_Widgets(QtCore.QObject):
    ...
    def populate_mainComboBox(self):
        parent = self.parent
        choices = list(self._observable_keys)
    ...
    def populate_subComboBox(self, param):
        ...
        if (
            param == "-"
            or sub_types_for_display(param) is None
            or not hasattr(self.parent, "mech")
            or self.widget["main_parameter"].currentText()
            in ["Density Gradient", "Heat Release Rate"]
        ):
            comboBox.hide()
        else:
            comboBox.show()

            if "total" in sub_types_for_display(param):
                comboBox.addItem("Total")

            if "species" in sub_types_for_display(param):
                for n, species in enumerate(self.parent.mech.gas.species_names):
                    comboBox.addItem(species)

            elif "rxn" in sub_types_for_display(param):
    ...
    def update_observable(self):
        ...
        self.parent.series.set(
            "observable", [mainComboBox.currentText(), subComboBox.currentIndex()]
        )
