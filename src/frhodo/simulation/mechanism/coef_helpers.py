...
"""Per-reaction coefficient bound helpers for the optimizer."""
import numpy as np
...
# Numerical floors / ceilings safe for log-space optimization.
...
def rates(rxn_coef_opt, mech):
    """Forward rate constants ``ln(k)`` at each requested ``(rxnIdx, T, P)``.

    Returns:
        Flat ``np.ndarray`` of natural-log rate constants, ordered by
        ``rxn_coef_opt`` and the embedded ``(T, P)`` arrays.
    """
    output = []
...
def set_bnds(mech, rxnIdx, keys, coefNames):
    """Per-coefficient lower/upper bounds for one reaction's Arrhenius coefs.

    For coefficients without user-supplied limits, returns sign-aware
    defaults: ``Ea`` is constrained not to change sign, ``A`` stays
    positive, and ``b`` lives inside the log-space range achievable
    over ``T ∈ [T_min, T_max]``.

    Returns:
        ``{"lower": np.ndarray, "upper": np.ndarray, "exist":
        np.ndarray}`` where ``exist[i] = [True, True]`` when the
        bound came from user limits and ``[False, False]`` when it
        was inferred.
    """
    ...
    for coefNum, (key, coefName) in enumerate(zip(keys, coefNames)):
        if coefName not in arrhenius_coefNames:
            continue  # skip anything not Arrhenius. Falloff follows this
        ...
        if np.isnan(coef_limits).any():
            ...
            if coefName == "activation_energy":
                if coef_x0 > 0:
                    ...
                    coef_bnds["lower"].append(
                        -Ru * T_min * np.log(max_pos_system_value)
                    )
                ...
                coefName, int
            ):  # ints will be falloff, they will be taken care of below
            ...
            if coefName == "activation_energy":
                if coef_x0 < 0:  # Ea shouldn't change sign
                    ...
                    coef_bnds["upper"].append(
                        -Ru * T_max * np.log(min_pos_system_value)
                    )
            elif coefName == "temperature_exponent":
...
def set_arrhenius_bnds(x0, coefNames):
    """Lower/upper bounds for a free-form Arrhenius coefficient vector.

    Args:
        x0: Reference coefficient values, used only to fix the sign of
            ``activation_energy`` bounds.
        coefNames: Names parallel to ``x0`` (``"activation_energy"``,
            ``"pre_exponential_factor"``, ``"temperature_exponent"``).

    Returns:
        ``[lower_list, upper_list]`` matching ``coefNames`` order.

    Raises:
        ValueError: an entry in ``coefNames`` is not a recognised
    ...
    for n, coefName in enumerate(coefNames):
        if coefName == "activation_energy":
            ...
            raise ValueError(
                f"set_arrhenius_bnds: unknown coefficient name {coefName!r}; "
                f"expected one of {arrhenius_coefNames}"
            )
