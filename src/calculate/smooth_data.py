...
def dual_tree_complex_wavelet_filter(
    signal,
    filt_signal=[],
    filter=None,
    filt_opts={},
    ...
    if len(filt_signal) > 0:  # if filtered signal is provided, pass
        ...
        filt_signal = wiener(
            signal_mod, mysize=filt_window, noise=4000
        )  # noise-power should be changed if used
    ...
    for n in range(max_iter):
        ...
        if len(wavelet_coef) == 2:
            rel_err = (
                np.abs(np.subtract(wavelet_coef[1], wavelet_coef[0]))
                / np.abs(wavelet_coef[0])
                * 100
            )
        ...
        if noise_type == "+-":  # outlier rejection
            ...
            if outlier_detection == "conf_int":
                ...
                # SE = std/np.sqrt(df+1)
                limits = np.sort(
                    [mean - std * t_crit, mean + std * t_crit]
                )  # limits is conf interval
                ...
                if n == 0:
                    percentile = 35
                q1, q3 = np.percentile(res, percentile), np.percentile(
                    res, 100 - percentile
                )
                iqr = q3 - q1
                cut_off = iqr * 1.5
            ...
            mask = np.greater(
                signal_mod, filt_signal
            )  # how it's done in Galloway paper
        elif noise_type == "-":
