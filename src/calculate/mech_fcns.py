...
class Chemical_Mechanism:
    ...
    def load_mechanism(self, path, silent=False):
        def chemkin2cantera(path):
            if path["thermo"] is not None:
                gas = ck2yaml.convert_mech(
                    path["mech"],
                    thermo_file=path["thermo"],
                    transport_file=None,
                    surface_file=None,
                    phase_name="gas",
                    out_name=path["Cantera_Mech"],
                    quiet=False,
                    permissive=True,
                )
            else:
        ...
        if "FAILED" in ct_out:
            ...
            output["message"].append(
                f'Wrote YAML mechanism file to {path["Cantera_Mech"]}.'
            )
            output["message"].append(
                f"Mechanism contains {n_species} species and {n_rxn} reactions."
            )
    ...
    def set_mechanism(self, mech_dict, species_dict={}, bnds=[]):
        ...
        for rxnIdx in range(len(mech_dict)):
            if "Arrhenius Reaction" == mech_dict[rxnIdx]["rxnType"]:
                ...
                rxn = ct.ChebyshevRate(
                    [mech_dict["Tmin"], mech_dict["Tmax"]],
        ...
        self.gas = ct.Solution(
            thermo="IdealGas", kinetics="GasKinetics", species=species, reactions=rxns
        )
    ...
    def set_rate_expression_coeffs(self, bnds=[]):
        ...
        for rxnIdx, rxn in enumerate(self.gas.reactions()):
            ...
            if rxn_type in ["Arrhenius Reaction", "Three Body Reaction"]:
                ...
                reset_coeffs = {
                    "Pmin": rxn.rate.pressure_range[0],
                    "Pmax": rxn.rate.pressure_range[1],
    ...
    def modify_reactions(
        self, coeffs, rxnIdxs=[]
    ):  # Only works for Arrhenius equations currently
        if not rxnIdxs:  # if rxnNums does not exist, modify all
            rxnIdxs = range(len(coeffs))
        else:
            if isinstance(
                rxnIdxs, (float, int)
            ):  # if single reaction given, run that one
                rxnIdxs = [rxnIdxs]
                ...
                for key in rate_dict.keys():
                    if "rate" in key:
                        for coefName in [
                            ...
                            if coeffs[rxnIdx][key][coefName] != eval(
                                f"rxn.rate.{key}.{coefName}"
                            ):
                                rxnChanged = True
    ...
    def M(self, rxn, TPX=[]):  # kmol/m^3
        ...
        if len(TPX) == 0:
            ...
            for i in range(0, len(T)):
                self.set_TPX(
                    T[i], P[i], X
                )  # IF MIXTURE CHANGES THEN THIS NEEDS TO BE VARIABLE
                M.append(get_M(rxn))
