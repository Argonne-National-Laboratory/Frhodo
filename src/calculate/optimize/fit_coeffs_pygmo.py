...
class falloff_parameters:  # based on ln_Fcent
    ...
    def objective(
        self, x_fit, grad=np.array([]), obj_type="obj_sum", aug_lagrangian=True
    ):
        ...
        if obj_type == "obj_sum":
            obj_val = penalized_loss_fcn(
                resid, a=self.loss_alpha, c=self.loss_scale
            ).sum()
        elif obj_type == "obj":
...
def fit_generic(
    ...
    if type(rxn.rate) == ct.ArrheniusRate:
        ...
        x0 = [
            ...
            for coefName in mech.coeffs_bnds[rxnIdx]["rate"]
        ]
