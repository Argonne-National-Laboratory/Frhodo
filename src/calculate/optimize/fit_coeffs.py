...
from calculate.optimize.misc_fcns import (
    set_arrhenius_bnds,
...
#                 'T3': {'-': [-1E14, -50],   '+': [50.0, 1E14]},
#                 'T1': {'-': [-1E14, -50],   '+': [50.0, 1E14]},
#                 'T2': {'-': [-1E4, 1E14],   '+': [-1E4, 1E14]}}
...
def fit_arrhenius(
    rates, T, x0=[], coefNames=default_arrhenius_coefNames, bnds=[], loss="linear"
):
...
def exp_safe(num, den):  # used to include divide by zero check, removed for numba
    ...
    res = np.zeros_like(
        x
    )  # since initialized to zero, values below min are already zero
...
class falloff_parameters:  # based on ln_Fcent
    def __init__(self, T, M, ln_k, x0, algo_options):
        ...
        self.penalty_vars = {
            "lambda": np.array([1, 1, 1, 1]),
            "rho": 1.0,  # rho is penalty parameter rho > 0
            "mu": 0.1,
        }  # mu > 0
        ...
        self.loss_alpha = self.algo["loss_fcn_param"][
            0
        ]  # warning unknown how well this functions outside of alpha=2, C=1
    ...
    def fit(self):
        ...
        p_bnds = np.concatenate(
            (p_bnds, set_arrhenius_bnds(self.p0[3:6], default_arrhenius_coefNames)),
            axis=1,
        )
        p_bnds[:, 1] = np.log(
            p_bnds[:, 1]
        )  # setting bnds to ln(A), need to do this better
        p_bnds[:, 4] = np.log(
            p_bnds[:, 4]
        )  # setting bnds to ln(A), need to do this better
    ...
    def jacobian(self, T, *x):  # defunct
        ...
        jac.append(
            bC * np.exp(np.abs(B)) * (1 - A) * T / T3**2 * np.exp(-T / T3)
        )  # dFcent/dB
        jac.append(
            bC * np.exp(np.abs(C)) * A * T / T1**2 * np.exp(-T / T1)
        )  # dFcent/dC
    ...
    def objective(
        self, x_fit, grad=np.array([]), obj_type="obj_sum", aug_lagrangian=False
    ):
        ...
        if (
            aug_lagrangian
        ):  # https://arxiv.org/pdf/2106.15044.pdf, https://www.him.uni-bonn.de/fileadmin/him/Section6_HIM_v1.pdf
            ...
            con.append(
                np.max([0, np.min(Fcent - self.Fcent_min)])
            )  # Fcent >= Fcent_min
            ...
            z = (
                0.5
                / rho
                * (((lamb - rho * con) ** 2 + 4 * rho * mu) ** 0.5 - (lamb - rho * con))
            )
            penalty = 0.0
            for zi in z:
    ...
    def Fcent_constraint(self, x_fit, grad=np.array([])):
        def f_fp(T, A, T3, T1, T2, fprime=False, fprime2=False):  # dFcent_dT
            ...
            if not fprime and not fprime2:
                ...
                fp = (
                    T2 * (T2 - 2 * T) / T**4 * np.exp(-T2 / T)
                    + (1 - A) / T3**2 * np.exp(-T / T3)
                    + A / T1**2 * np.exp(-T / T1)
                )
                return f, fp
        ...
        try:
            T_deriv_eq_0 = root_scalar(
                lambda T: f_fp(A, T3, T1, T2),
                x0=(Tmax + Tmin) / 4,
                x1=3 * (Tmax + Tmin) / 4,
                method="secant",
            )
        ...
        if grad.size > 0:
            grad[:] = self.constraint_gradient(
                x, numerical_gradient=self.Fcent_constraint
            )
    ...
    def Arrhenius_constraint(self, x_fit, grad=np.array([])):
        ...
        if grad.size > 0:
            grad[:] = self.constraint_gradient(
                x, numerical_gradient=self.Arrhenius_constraint
            )
    ...
    def calc_s(self, x, grad=[]):
        ...
        if (y < min_pos_system_value).all():
            ...
            )  # TODO: MAKE THIS BETTER running into problem when s is zero, this is a janky workaround
...
class Troe:
    ...
    def fit(self):
        ...
        self.x0[idx_A] = np.log(self.x0[idx_A])  # convert to ln_A
        ...
        if self.pool is not None:
            ...
            for i, p0 in enumerate(x0):
                falloff = falloff_parameters(
                    T, M, ln_k, p0, self.algorithm_options[0]
                )  # GN_CRS2_LM LN_SBPLX
        ...
        # use_str = 'using G'
        if res["fval"] < HoF["obj_fcn"] and len(res["x"]) == len(
            x
        ):  # this length check shouldn't be needed
            x = res["x"]
            # use_str = 'using L'
    ...
    def fit_arrhenius_rates(self):
        ...
        ln_k_Arrhenius = []
        idx = [-1]
        for i, (x0_idx, arrhenius_type) in enumerate(
            zip([[0, 1, 2], [3, 4, 5]], ["low_rate", "high_rate"])
        ):
            # set coefNames to be optimized based on GUI
            ...
            idx = np.arange(idx[-1], idx[-1] + len(x_idx)) + 1
            coefNames = [
                coef.split("_0")[0].split("_inf")[0] for coef in self.coefNames[idx]
            ]

            if len(idx) > 0:
                x_temp[x_idx] = fit_arrhenius(
                    rates[idx],
                    T[idx],
                    x0=x0[x0_idx],
                    coefNames=coefNames,
...
def fit_generic(
    ...
    if type(rxn.rate) is ct.ArrheniusRate:
        ...
        if (rxn.reaction_type == "three-body") and (
...
def fit_coeffs(
    ...
    if len(coefNames) == 0:
        return  # if not coefs being optimized in rxn, return
...
def debug(mech):
    ...
    rates = [1529339.05689338, 1548270.86688399, 1567437.0352583] * np.array(
        [1.000002, 1.00002, 1]
    )
...
if __name__ == "__main__":
    ...
    T = [1513.8026716]
    x0 = [1439225332.24, 5.8499038e276, -71.113552]
    coefNames = ["pre_exponential_factor"]
    bnds = [[2.4424906541753446e-16], [1.7976931348623155e288]]
