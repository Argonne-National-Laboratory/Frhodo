...
def get_log_posterior_density(PE_object, parameters):
    return PE_object.getLogP(
        parameters, runBoundsCheck=False
    )  # bounds are already checked prior to this. Differing methods were causing problems. No reason to make them match, just disable
...
class CheKiPEUQ_Frhodo_interface:
    def __init__(self, input_dict):
        ...
        self.Bayesian_dict = {}
        self.Bayesian_dict["pars_uncertainty_distribution"] = input_dict[
            "opt_settings"
        ][
            "bayes_dist_type"
        ]  # options can be 'Auto', 'Gaussian' or 'Uniform'.
        self.Bayesian_dict["rate_constants_initial_guess"] = deepcopy(
            input_dict["rxn_rate_opt"]["x0"]
        )
        self.Bayesian_dict["rate_constants_lower_bnds"] = deepcopy(
            input_dict["rxn_rate_opt"]["bnds"]["lower"]
        )
        self.Bayesian_dict["rate_constants_upper_bnds"] = deepcopy(
            input_dict["rxn_rate_opt"]["bnds"]["upper"]
        )
        ...
        )  # From Jan 2021, we are setting [True True] for each rate_constant.

        self.Bayesian_dict["rate_constants_parameters_changing"] = deepcopy(
            input_dict["coef_opt"]
        )
    ...
    def evaluate(self, CheKiPEUQ_eval_dict):
        Bayesian_dict = self.Bayesian_dict
        Bayesian_dict["rate_constants_current_guess"] = deepcopy(
            CheKiPEUQ_eval_dict["log_opt_rates"]
        )
        Bayesian_dict["rate_constants_parameters_current_guess"] = deepcopy(
            CheKiPEUQ_eval_dict["x"]
        )
        ...
        Bayesian_dict["last_obs_sim_interp"] = np.concatenate(
            CheKiPEUQ_eval_dict["output_dict"]["obs_sim_interp"], axis=0
        )
        Bayesian_dict["observed_data"] = np.concatenate(
            CheKiPEUQ_eval_dict["output_dict"]["obs_exp"], axis=0
        )
        obs_data_bnds = np.concatenate(
            CheKiPEUQ_eval_dict["output_dict"]["obs_bounds"], axis=0
        ).T
        ...
        if (
            CheKiPEUQ_eval_dict["iteration_num"] == 0
            and "obj_fcn_initial" not in Bayesian_dict
        ):
