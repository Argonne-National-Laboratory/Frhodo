...
mumpceProjectObject.model_parameter_info = np.array(
    [
        {
            "parameter_number": 0,
        ...
        },
        {
            ...
            "parameter_value": 1.0,
        },  # The parameter values are not used in making these contour plots.
        {
            "parameter_number": 2,
...
)  # This must be made into a numpy array.
mumpceProjectObject.active_parameters = np.array(
    [0, 1, 2, 4, 6]
)  # this must be made into a numpy array.
# mumpceProjectObject.set_active_parameters = [0, 1, 2, 4, 6]
Posterior_mu_vector = np.array(
    [-0.58888733, 1.1200355, 0.00704044, -1.62385888, 0.80439847]
)  # this must be made into a numpy array. #This will become solution.x
Posterior_cov_vector = np.array(
    [
        [0.0148872, -0.01894579, -0.01047339, 0.01325883, 0.04734254],
...
)  # This will become solution.cov. It does not need to be a numpy array, but we make it one for consistency.

Prior_mu_vector = np.array(
    [-0.98888733, 0.8200355, 0.01204044, -7.02385888, 0.40439847]
)
Prior_cov_vector = 10 * Posterior_cov_vector

mumpceSolutionsObject = mumpceSolution.Solution(
    Posterior_mu_vector,
    Posterior_cov_vector,
    initial_x=Prior_mu_vector,
    initial_covariance=Prior_cov_vector,
)
...
contour_settings_custom[
    "num_y_ticks"
] = 3  # Note that this is a *request* When it's not fulfilled exactly, the user can play with the number.
contour_settings_custom[
    "num_x_ticks"
] = 3  # Note that this is a *request* When it's not fulfilled exactly, the user can play with the number.
...
contour_settings_custom[
    "zoom_std_devs"
] = 4.5  # These are in units of standard deviations of the prior.
mumpceProjectObject.plot_pdfs(
    mumpceProjectObject.pairsOfParameterIndices,
    contour_settings_custom=contour_settings_custom,
)
