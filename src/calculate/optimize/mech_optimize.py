...
class Multithread_Optimize:
    ...
    def start_threads(self):
        ...
        self.time_between_plots = (
            0.0  # maximum update rate, updated based on time to plot
        )
        ...
        if len(self.shocks2run) == 0:  # optimize current shock if nothing selected
            ...
            if (
                not parent.load_full_series_box.isChecked()
            ):  # TODO: May want to remove this limitation in future
                parent.log.append(
                    '"Load Full Series Into Memory" must be checked for optimization of multiple experiments\n'
                )
                return
            elif len(parent.series_viewer.data_table) == 0:
                parent.log.append(
                    "Set Series in Series Viewer and select experiments\n"
                )
                return

        if len(self.shocks2run) == 0:
            return  # if no shocks to run return, not sure if necessary anymore
        ...
        for (
            shock
        ) in self.shocks2run:  # TODO NEED TO UPDATE THIS FOR UNCERTAINTIES AND WEIGHTS
            ...
            if opt_options["obj_fcn"]["type"] == "Residual":
                weight_var = [
                    shock[key]
                    for key in ["weight_max", "weight_min", "weight_shift", "weight_k"]
                ]
                ...
                if np.isnan(np.hstack(unc_var)).any():
                    ...
                    shock["uncertainties"] = unc_fcn(
                        shock["exp_data"][:, 0], shock, calcWeights=True
                    )
    ...
    def _set_coef_opt(self):
        ...
        for rxnIdx, rxn in enumerate(mech.gas.reactions()):  # searches all rxns
            if not mech.rate_bnds[rxnIdx]["opt"]:
                continue  # ignore fixed reactions
            ...
            for bndsKey, subRxn in mech.coeffs_bnds[rxnIdx].items():
                for coefIdx, (coefName, coefDict) in enumerate(subRxn.items()):
                    if coefDict["opt"]:
                        coefKey, bndsKey = mech.get_coeffs_keys(
                            rxn, bndsKey, rxnIdx=rxnIdx
                        )
                        coef_opt.append(
                            {
                                "rxnIdx": rxnIdx,
                                "key": {"coeffs": coefKey, "coeffs_bnds": bndsKey},
                                "coefIdx": coefIdx,
                                "coefName": coefName,
                            }
                        )
    ...
    def _set_rxn_coef_opt(self, min_T_range=500, min_P_range_factor=2):
        ...
        if (
            T_bnds[1] - T_bnds[0] < min_T_range
        ):  # if T_range isn't large enough increase it
        ...
        for rxn_coef in rxn_coef_opt:
            ...
            rxn_coef["coef_x0"] = []
            for coefNum, (key, coefName) in enumerate(
                zip(rxn_coef["key"], rxn_coef["coefName"])
            ):
                coef_x0 = mech.coeffs_bnds[rxnIdx][key["coeffs_bnds"]][coefName][
                    "resetVal"
                ]
            ...
            if type(rxn.rate) is ct.ArrheniusRate:
                ...
                for coef_type in ["low_rate", "high_rate"]:
                    ...
                    if coef_type == "low_rate":
                        rxn_coef["P"].append(
                            np.ones(n_coef) * P[0]
                        )  # will evaluate LPL if LPL is constrained, else this value
                    elif coef_type == "high_rate":
                        rxn_coef["P"].append(
                            np.ones(n_coef) * P[-1]
                        )  # will evaluate HPL if HPL is constrained, else this value
            ...
            rxn_coef["X"] = shock_conditions["thermo_mix"][
                0
            ]  # TODO: IF MIXTURE COMPOSITION FOR DUMMY RATES MATTER CHANGE HERE
            rxn_coef["is_falloff_limit"] = np.array(
                [False] * len(rxn_coef["T"])
            )  # this only has meaning for falloff equations
    ...
    def _set_rxn_rate_opt(self):
        ...
        for i, rxn_coef in enumerate(self.rxn_coef_opt):
            ...
            if type(rxn.rate) in [
                ...
                ct.TroeRate,
                ct.SriRate,
            ]:  # if falloff, change arrhenius rates to LPL/HPL if they are not constrained
                        ...
                        if (
                            ...
                            if type(rxn.rate) in [
                                ...
                                for ArrheniusCoefName in arrhenius_coefNames:
                                    x.append(
                                        mech.coeffs_bnds[rxnIdx][coef_type_key][
                                            ArrheniusCoefName
                                        ]["resetVal"]
                                    )
            ...
            rxn_coef_bnds = np.sort(
                np.log(rxn_coef_bnds), axis=0
            )  # operate on ln and scale
    ...
    def _update_gas(self):  # TODO: What happens if a second optimization is run?
        ...
        rxns_changed = []
        i = 0
        for rxn_coef_idx, rxn_coef in enumerate(
            self.rxn_coef_opt
        ):  # TODO: RXN_COEF_OPT INCORRECT FOR CHANGING RXN TYPES
            rxnIdx = rxn_coef["rxnIdx"]
            rxn = mech.gas.reaction(rxnIdx)
            if type(rxn.rate) is ct.ArrheniusRate:
            ...
            if type(rxn.rate) in [ct.FalloffRate, ct.TroeRate, ct.SriRate]:
                ...
                rxn_coef["coefIdx"].extend(
                    range(0, 4)
                )  # extend to include falloff coefficients
                rxn_coef["coefName"].extend(
                    range(0, 4)
                )  # extend to include falloff coefficients
                rxn_coef["key"].extend(
                    [
                        {
                            "coeffs": "falloff_parameters",
                            "coeffs_bnds": "falloff_parameters",
                        }
                        for _ in range(0, 4)
                    ]
                )
                ...
                for key in ["low_rate", "high_rate"]:
                    for coefName in arrhenius_coefNames:
                        rxn_coef["key"][n][
                            "coeffs"
                        ] = key  # change key value to match new reaction type
                        mech.coeffs[rxnIdx][key][coefName] = rxn_coef["coef_x0"][
                            n
                        ]  # updates new arrhenius values
        ...
        if generate_new_mech:
            ...
            parent.save.chemkin_format(
                mech.gas, parent.path_set.optimized_mech(file_out="recast_mech")
            )
    ...
    def update(self, result, writeLog=True):
        ...
        if writeLog:
            ...
            log_str = (
                f"\t{opt_type.upper()} {i:^5d}{obj_fcn_space}{obj_fcn_str:^s}"
        ...
        if timer() - self.last_plot_timer > self.time_between_plots:
            ...
            if result["ind_var"] is None and result["observable"] is None:
                ...
                self.parent.plot.signal.update_sim(
                    result["ind_var"][:, 0], result["observable"][:, 0]
                )
    ...
    def on_worker_done(self, result):
        ...
        for opt_type, res in result.items():
            ...
            parent.log.append(
                "\telapsed time:\t{:.2f}".format(res["time"]), alert=False
            )
            parent.log.append(
                "\tAvg Std Residual:\t{:.3e}".format(res["fval"]), alert=False
            )
            parent.log.append(
                "\topt iters:\t\t{:.0f}".format(res["nfev"] + 1), alert=False
            )
            parent.log.append(
                "\tshock evals:\t{:.0f}".format(total_shock_eval), alert=False
            )
        ...
        parent.app.alert(
            parent, 5 * 1000
        )  # make application alert for x millisec on completion
