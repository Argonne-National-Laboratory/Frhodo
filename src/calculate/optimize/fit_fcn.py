...
def initialize_parallel_worker(mech_dict, species_dict, coeffs, coeffs_bnds, rate_bnds):
    ...
    with contextlib.redirect_stderr(io.StringIO()):
        with contextlib.redirect_stdout(io.StringIO()):
            mech.set_mechanism(
                mech_dict, species_dict
            )  # load mechanism from yaml text in memory
...
def update_mech_coef_opt(mech, coef_opt, x):
    ...
    for i, idxDict in enumerate(coef_opt):
        ...
        ):  # limits mech changes. Should increase speed a little
            if (
                type(mech.coeffs[rxnIdx][coeffs_key]) is tuple
            ):  # don't know why but sometimes reverts to tuple
...
def calculate_residuals(args_list):
    def resid_func(
        ...
        def calc_exp_bounds(t_sim, t_exp):
            t_bounds = [
                max([t_sim[0], t_exp[0]])
            ]  # Largest initial time in SIM and Exp
            t_bounds.append(
                min([t_sim[-1], t_exp[-1]])
            )  # Smallest final time in SIM and Exp
            # Values within t_bounds
            exp_bounds = np.where(
                np.logical_and((t_exp >= t_bounds[0]), (t_exp <= t_bounds[1]))
            )[0]
        ...
        if scale == "Linear":
            ...
            if verbose and opt_type == "Bayesian":
                obs_exp = np.log10(
                    np.abs(obs_exp[ind])
                ).squeeze()  # squeeze to remove extra dim
        ...
        loss = penalized_loss_fcn(
            resid, a=loss_alpha, c=resid_outlier, use_penalty=loss_penalty
        )
        ...
        if verbose:
            ...
            if (
                opt_type == "Bayesian"
            ):  # need to calculate aggregate weights to reduce outliers in bayesian
                ...
                loss_weights = (
                    loss / SSE
                )  # comparison is between selected loss fcn and SSE (L2 loss)
    ...
    SIM, verbose = mech.run(
        var["name"], var["t_end"], T_reac, P_reac, mix, **SIM_kwargs
    )
    ...
    if not np.any(var["t_unc"]):
        ...
        t_unc_OoM = np.mean(
            OoM(var["t_unc"])
        )  # Do at higher level in code? (computationally efficient)
        ...
        res = minimize_scalar(
            time_adj_func, bounds=var["t_unc"] / 10**t_unc_OoM, method="bounded"
        )
...
class Fit_Fun:
    def __init__(self, input_dict):
        ...
        self.opt_settings = {
            "obj_fcn_type": self.parent.optimization_settings.get("obj_fcn", "type"),
            ...
            "bayes_dist_type": self.parent.optimization_settings.get(
                "obj_fcn", "bayes_dist_type"
            ),
            "bayes_unc_sigma": self.parent.optimization_settings.get(
                "obj_fcn", "bayes_unc_sigma"
            ),
        }
        ...
        if (
            self.opt_settings["obj_fcn_type"] == "Bayesian"
        ):  # initialize Bayesian_dictionary if Bayesian selected
    ...
    def __call__(self, s, optimizing=True):
        ...
        if self.multiprocessing and len(self.shocks2run) > 1:
            args_list = (
                (var_dict, self.coef_opt, x, shock) for shock in self.shocks2run
            )
        ...
        if loss_alpha == 3.0:
            if np.size(loss_resid) <= 2:  # optimizing only a few experiments, use SSE
                ...
                res = minimize_scalar(
                    loss_alpha_fcn, bounds=[-100, 2], method="bounded"
                )
                loss_alpha = res.x
        ...
        obj_fcn = self.calculate_obj_fcn(
            ...
            log_opt_rates,
            output_dict,
            obj_fcn_type=self.opt_settings["obj_fcn_type"],
        )
    ...
    def calculate_obj_fcn(
        ...
        if np.size(loss_resid) == 1:  # optimizing single experiment
            ...
            if obj_fcn_type == "Residual":
                loss_exp = penalized_loss_fcn(
                    loss_resid - loss_min, a=alpha, c=loss_outlier
                )
            else:  # otherwise do not include penalty for Bayesian
                loss_exp = penalized_loss_fcn(
                    loss_resid - loss_min, a=alpha, c=loss_outlier, use_penalty=False
                )
