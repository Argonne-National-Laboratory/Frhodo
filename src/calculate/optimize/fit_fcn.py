...
def calculate_residuals(args_list):
    def resid_func(
        ...
        loss_weights, C, alpha = adaptive_weights(
            resid, weights, C_scalar=loss_c, alpha=loss_alpha
        )
        ...
        if verbose:
            output = {
                ...
                "aggregate_weights": weights * loss_weights,
    ...
    def calculate_obj_fcn(
        ...
        if np.size(loss_resid) == 1:  # optimizing single experiment
            ...
            exp_loss_weights, C, alpha = adaptive_weights(
                loss_resid - loss_min, weights=np.array([]), C_scalar=C, alpha=alpha
            )
        ...
        if obj_fcn_type == "Residual":
            ...
            if np.size(loss_resid) == 1:  # optimizing single experiment
                ...
                exp_loss_weights, C, alpha = adaptive_weights(
                    loss_resid, weights=np.array([]), C_scalar=C, alpha=alpha
                )
