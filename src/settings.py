...
class Path:
    ...
    def mech(self):
        ...
        for file in parent.path["mech_main"].glob("*"):
            ...
            if ext == ".tran":  # currently unused, but store transport files anyways
                ...
                ".mech",
                ".inp",
            ]:  #  '.ctml', '.xml', # TODO: enable ctml and xml format
    ...
    def shock_paths(self, prefix, ext, max_depth=2):
        ...
        for file in parent.path["exp_main"].rglob("*"):
            ...
            match = re.search(
                prefix + ".*\." + ext + "$", file.name
            )  # search for prefix + ext
            if match:
        ...
        if self.sim_num == 1:
            ...
            if self.sim_num == 2:
                ...
                for entry in parent.path["output_dir"].glob("*"):
                    if entry.is_file():
                        if (
                            len(re.findall("Sim \d+ - ", entry.name)) > 0
...
class experiment:
    ...
    def read_tranter_exp_v1(self, lines):
        ...
        mix = {}
        for key in [
            item[0] for item in self.config.items("Mixture")
        ]:  # search all keys in section
        ...
        if self.config.has_option(
            "Expt Params", "VelatObs"
        ):  # Tranter exp file v1.1 gives velocity
    ...
    def read_tranter_exp_v0(self, lines):
        ...
        parameters[
            "P4"
        ] = 1.0  # Arbitrarily assign. Does not matter for general functionality
...
def double_sigmoid(x, A, k, x0):  # A = extrema, k = inverse growth rate, x0 = shifts
    def sig(x):  # Numerically stable sigmoid function
        ...
        eval[eval > 0] = np.clip(
            eval[eval > 0], min_pos_system_value, max_pos_system_value
        )
        eval[eval < 0] = np.clip(
            eval[eval < 0], -max_pos_system_value, -min_pos_system_value
        )
...
class series:
    ...
    def _create_shock(self, num, shock_path):
        ...
        shock = {
            "num": num,
            "path": deepcopy(shock_path),
            "include": False,
            "series_name": self.name[-1],
            "run_SIM": True,  # TODO: make run_SIM a trigger to calculate or not
            # Shock Parameters
            "T1": np.nan,
            "P1": np.nan,
            "u1": np.nan,
    ...
    def add_series(self):  # need to think about what to do when mech changes, anything?
        parent = (
            self.parent
        )  # how do I know when I'm looking at a new compound? base on mech change?
    ...
    def clear_shocks(self):
        if self.parent.load_full_series:
            return
    ...
    def weights(self, time, shock=[], calcIntegral=True):
        ...
        if (
            ...
            integral = integrate.cumtrapz(weights, time)[
                -1
    ...
    def _integrate_weights(self, shock):  # Defunct, but it works!
        ...
        integral, err = integrate.quad(
            weights, t0, tf
        )  # integrate from t0 to tf using gaussian quad
    ...
    def uncertainties(self, time, shock=[], calcWeights=False):
        ...
        parameters = [
            shock[key] for key in ["unc_max", "unc_min", "unc_shift", "unc_k"]
        ]
        if np.isnan(
            np.hstack(parameters)
        ).any():  # if weight parameters aren't set, default to gui
        ...
        if calcWeights:
            weights = shock["weights"] = double_sigmoid(
                time, [0, 1, 0], [0, 0], unc_cutoff
            )
            integral = integrate.cumtrapz(weights, time)[
                -1
            ]  # based on weights at data points
            ...
            if self.parent.exp_unc.unc_type == "%":
                shock["abs_uncertainties"] = np.sort(
                    [obs_data / (1 + unc), obs_data * (1 + unc)], axis=0
                ).T
            else:
                shock["abs_uncertainties"] = np.sort(
                    [obs_data - unc, obs_data + unc], axis=0
                ).T
    ...
    def smoothed_data(self, signal):
        def calculate_C(signal):
            ...
            C *= 10 ** (
                OoM(max_signal) + 2
            )  # scaling factor TODO: + 1 looks loglike, + 2 linear like
        ...
        obs = np.sign(obs) * np.log10(
            1 + np.abs(obs / C)
        )  # apply bisymlog prior to smoothing
        obs = dual_tree_complex_wavelet_filter(
            obs, lvls=lvls
        )  # smooth transformed data
        obs = (
            np.sign(obs) * C * (np.power(10, np.abs(obs)) - 1)
    ...
    def set(self, key, val=[], **kwargs):
        ...
        if key == "exp_data":
            ...
            if (
                opt_type != "Bayesian"
                or parent.plot.signal.unc_shading != "Smoothed Signal"
                or len(shock["exp_data"]) == 0
                or shock["wavelet_lvls"] == lvls
            ):
                return
            ...
            if lvls == 1:
                ...
                shock["exp_data_smoothed"] = self.smoothed_data(
                    shock["exp_data"].copy()
                )

        elif (
            key == "series_name"
        ):  # being called many times when weights changing, don't know why yet
    ...
    def thermo_mix(self, shock=[]):
        ...
        if len(shock) == 0:  # if no shock given, assume shock is display_shock
            ...
            parent.display_shock["thermo_mix"] = mix = deepcopy(
    ...
    def change_shock(self):
        ...
        if np.isnan(
            np.hstack(parameters)
        ).any():  # if weight parameters aren't set, create from gui
        ...
        parameters = [
            shock[key]
            for key in ["unc_max", "unc_min", "unc_shift", "unc_k", "unc_cutoff"]
        ]
        if np.isnan(
            np.hstack(parameters)
        ).any():  # if weight parameters aren't set, create from gui
