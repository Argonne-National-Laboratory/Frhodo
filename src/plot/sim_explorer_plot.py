...
class Plot(Base_Plot):
    ...
    def create_canvas(self):
        ...
        for n, ax in enumerate(self.ax):
            ...
            for i in range(max_lines):
                ax.item["property"].append(
                    ax.add_line(mpl.lines.Line2D([], [], ls=ls[n], c=color[n][i]))
                )
    ...
    def toggle_y2_axis(self, show_y2=False):
        if show_y2:
            ...
            self.fig.subplots_adjust(
                left=0.06, bottom=0.065, right=0.94, top=0.97, hspace=0, wspace=0.12
            )
        else:
    ...
    def update(self, data, labels=[], label_order=[], update_lim=True):
        ...
        for n, key in enumerate(data):
            ...
            if isinstance(
                axes.get_legend(), mpl.legend.Legend
            ):  # if legend exists, remove it
                leg = axes.get_legend()
            ...
            for i, yData in enumerate(
                axisData
            ):  # update lines, won't update if empty list
    ...
    def _get_data(self, axes):
        ...
        if "property" in axes.item:
            ...
            for line in axes.item["property"]:
                data["y"] = np.append(
                    data["y"], line.get_ydata()
                )  # combine all lines on y
