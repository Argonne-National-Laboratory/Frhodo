...
class Tree(QtCore.QObject):
        ...
        for rxnIdx, rxn in enumerate(mech.gas.reactions()):
            ...
            if type(rxn.rate) is ct.ArrheniusRate:
                ...
                for key in ["high", "low"]:
                    ...
                    for coefName, coefAbbr in coef_abbreviation.items():
                        coeffs.append(
                            [f"{coefAbbr}_{key}", coefName, mech.coeffs[rxnIdx][n]]
                        )
                ...
                coeffs_order = [
                    ...
                    5,
                    3,
                ]  # order coeffs into A_high, n_high, Ea_high, A_low
    ...
    def _set_mech_widgets(self, sender):
        ...
        if rxn["type"] in ["Arrhenius Reaction", "Plog Reaction", "Falloff Reaction"]:
            ...
            widget.uncValBox.valueChanged.connect(
                self.update_uncertainties
            )  # no update between F and %

            len_coef = len(rxn["coeffs_order"])
            tree.rxn[rxnNum].update(
                {
                    "coef": rxn["coeffs"],
            ...
            for coefNum in rxn["coeffs_order"]:
                ...
                L1.info["row"].append(
                    {"item": QtGui.QStandardItem(""), "widget": widget}
                )
                L1.appendRow([L1.info["row"][-1]["item"]])
                mIndex = self.proxy_model.mapFromSource(
                    L1.info["row"][-1]["item"].index()
                )
                tree.setIndexWidget(mIndex, L1.info["row"][-1]["widget"])
    ...
    def update_uncertainties(self, event=None, sender=None):
        ...
        for rxnNum in rxnNumRange:  # update all rate uncertainties
            ...
            if rxn["rxnType"] not in [
                "Arrhenius Reaction",
                "Plog Reaction",
                "Falloff Reaction",
            ]:  # skip if not allowable type
    ...
    def update_coef_rate_from_opt(self, coef_opt, x):
        ...
        conv_type = "Cantera2" + self.mech_tree_type
        for i, idxDict in enumerate(
            coef_opt
        ):  # set changes to both spinboxes and backend coeffs
    ...
    def update_mech_reset_value(
        self, event
    ):  # TODO WHEN BOXES ARE CREATED UNCERTAINTIES ARE BEING CALLED TWICE
        parent = self.parent()
        sender = self.sender()
        rxnNum, coefNum, coefName = (
            sender.info["rxnNum"],
        ...
        self.update_uncertainties(
            event, sender
        )  # update uncertainties based on new reset value
    ...
    def update_box_reset_values(self, rxnNum=None):
        ...
        for rxnNum in rxnNumRange:
            ...
            if (
                rxn["rxnType"]
                not in ["Arrhenius Reaction", "Plog Reaction", "Falloff Reaction"]
    ...
    def _reset_all(self):
        ...
        for rxn in parent.mech_tree.rxn:
            if (
                ...
                continue  # only reset Arrhenius boxes
    ...
    def _setDependence(self, rxn, isDependent):
        ...
        if isDependent:
            ...
            for valueBox, formulaBox in zip(rxn["valueBox"], rxn["formulaBox"]):
                silentSetValue(
                    valueBox, float(formulaBox.text())
                )  # propogate changes to valuebox
                formulaBox.hide()
                valueBox.show()
        ...
        for rxnNum, rxn in enumerate(parent.mech_tree.rxn):
            if rxn["dependent"]:
                ...
                for box in rxn["formulaBox"]:
                    ...
                    reset_value = mech.coeffs_bnds[rxnNum][bnds_key][coefName][
                        "resetVal"
                    ]
                    if reset_value == 0.0:
...
class ScientificLineEditReadOnly(QLineEdit):
    ...
    def mousePressEvent(
        self, event
    ):  # Yes, basically I made this whole line edit for this. It's called efficiency
        self.selectAll()
        # self.copy()
...
class ScientificLineEdit(QLineEdit):
    ...
    def applyFormula(self, emit, adjustment="*1.0"):
        ...
        for name in names:
            ...
            if (
                subRxnNum == rxnNum
            ):  # if rxnNum matches current value, set value to initial
        ...
        value = parent.convert_units._arrhenius(rxnNum, deepcopy([coeffs]), conv_type)[
            0
        ][2]
