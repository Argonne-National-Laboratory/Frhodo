...
class GUI_Config(yaml.YAML):
    ...
    def to_yaml(self, dest=None):
        ...
        toFlowMap = [
            ["Experiment Settings", "temperature units"],
            ["Experiment Settings", "pressure units"],
        ]
        toFlowList = [
            ["Optimization Settings", "weight function", "min"],
...
class GUI_settings:
    ...
    def load(self):
        ...
        settings = {
            "directory": self.cfg["Directory Settings"],
        ...
        set_box(
            parent.T1_units_box, f"[{settings['exp']['temperature units']['zone 1']}]"
        )
        set_box(
            parent.T2_units_box, f"[{settings['exp']['temperature units']['zone 2']}]"
        )
        set_box(
            parent.T5_units_box, f"[{settings['exp']['temperature units']['zone 5']}]"
        )
        ...
        set_box(
            parent.end_time_value_box,
            settings["reactor"]["simulation end time"]["value"],
        )
        set_box(
            parent.end_time_units_box,
            f"[{settings['reactor']['simulation end time']['units']}]",
        )
        set_box(parent.ODE_solver_box, settings["reactor"]["ODE solver"])
        set_box(
            parent.sim_interp_factor_box,
            settings["reactor"]["simulation interpolation factor"],
        )
        ...
        set_box(
            parent.bayes_dist_type_box, settings["opt"]["Bayesian distribution type"]
        )
        set_box(
            parent.bayes_unc_sigma_box, settings["opt"]["Bayesian uncertainty sigma"]
        )
        ...
        for opt_type in ["global", "local"]:
            ...
            if opt_type == "global":
                set_box(
                    parent.global_opt_enable_box, settings["opt"]["enabled"][opt_type]
                )
                set_box(
                    parent.global_opt_choice_box, settings["opt"]["algorithm"][opt_type]
                )
                set_box(
                    widget["initial_pop_multiplier"],
                    settings["opt"]["initial population multiplier"][opt_type],
                )

            else:
                set_box(
                    parent.local_opt_enable_box, settings["opt"]["enabled"][opt_type]
                )
                set_box(
                    parent.local_opt_choice_box, settings["opt"]["algorithm"][opt_type]
                )

            set_box(widget["initial_step"], settings["opt"]["initial step"][opt_type])
            set_box(
                widget["stop_criteria_type"],
                settings["opt"]["stop criteria type"][opt_type],
            )
            set_box(
                widget["stop_criteria_val"],
                settings["opt"]["stop criteria value"][opt_type],
            )
            set_box(
                widget["xtol_rel"], settings["opt"]["relative x tolerance"][opt_type]
            )
            set_box(
                widget["ftol_rel"], settings["opt"]["relative fcn tolerance"][opt_type]
            )
        ...
        parent.unc_type_box.setCurrentText(
            settings["opt"]["uncertainty function"]["type"]
        )
        ...
        set_box(
            parent.unc_shading_box, settings["opt"]["uncertainty function"]["shading"]
        )
        set_box(
            parent.wavelet_levels_box,
            settings["opt"]["uncertainty function"]["wavelet_lvls"],
        )

        ## Set Plot Settings ##
        parent.plot.signal._set_scale(
            "x", settings["plot"]["x-scale"], parent.plot.signal.ax[1], True
        )
        parent.plot.signal._set_scale(
            "y", settings["plot"]["y-scale"], parent.plot.signal.ax[1], True
        )
    ...
    def save(self, save_all=False):
        ...
        for i in [1, 2, 5]:
            T_unit = (
                eval(f"parent.T{i}_units_box.currentText()").lstrip("[").rstrip("]")
            )
            P_unit = (
                eval(f"parent.P{i}_units_box.currentText()").lstrip("[").rstrip("]")
            )
            settings["exp"]["temperature units"][f"zone {i}"] = T_unit
            settings["exp"]["pressure units"][f"zone {i}"] = P_unit

        # Set Incident Velocity Units
        settings["exp"]["velocity units"] = (
        ...
        settings["reactor"]["simulation end time"][
            "value"
        ] = parent.end_time_value_box.value()
        ...
        settings["reactor"][
            "simulation interpolation factor"
        ...
        ] = parent.obj_fcn_type_box.currentText()
        settings["opt"][
            "objective function scale"
        ] = parent.obj_fcn_scale_box.currentText()
        settings["opt"]["loss function alpha"] = parent.loss_alpha_box.currentText()
        settings["opt"]["loss function c"] = parent.loss_c_box.value()
        settings["opt"][
            "Bayesian distribution type"
        ] = parent.bayes_dist_type_box.currentText()
        settings["opt"][
            "Bayesian uncertainty sigma"
        ] = parent.bayes_unc_sigma_box.value()
        ...
        for opt_type in ["global", "local"]:
            ...
            if opt_type == "global":
                settings["opt"]["enabled"][
                    opt_type
                ] = parent.global_opt_enable_box.isChecked()
                settings["opt"]["algorithm"][
                    opt_type
                ] = parent.global_opt_choice_box.currentText()
                settings["opt"]["initial population multiplier"][opt_type] = widget[
                    "initial_pop_multiplier"
                ].value()
            else:
                settings["opt"]["enabled"][
                    opt_type
                ] = parent.local_opt_enable_box.isChecked()
                settings["opt"]["algorithm"][
                    opt_type
                ] = parent.local_opt_choice_box.currentText()

            settings["opt"]["initial step"][opt_type] = widget["initial_step"].value()
            settings["opt"]["stop criteria type"][opt_type] = widget[
                "stop_criteria_type"
            ].currentText()
            settings["opt"]["stop criteria value"][opt_type] = widget[
                "stop_criteria_val"
            ].value()
            settings["opt"]["relative x tolerance"][opt_type] = widget[
                "xtol_rel"
            ].value()
            settings["opt"]["relative fcn tolerance"][opt_type] = widget[
                "ftol_rel"
            ].value()
        ...
        # Update uncertainty function
        settings["opt"]["uncertainty function"][
            "type"
        ] = parent.unc_type_box.currentText()
        ...
        settings["opt"]["uncertainty function"][
            "shading"
        ] = parent.unc_shading_box.currentText()
        settings["opt"]["uncertainty function"][
            "wavelet_lvls"
        ] = parent.wavelet_levels_box.value()
